//! Workload-cgroup management for the VM driver.
//!
...
/// [`CgroupPath`] / [`CgroupPathError`] RELOCATED into `overdrive-core`
/// per ADR-0082 §D2 (Amendment 2026-08-12, gap 1, GH #42) — `VmConfig`
/// (`overdrive-core::vm::config`) needs the type as a field, and
/// `overdrive-core` cannot depend on this crate. Re-exported here
/// VERBATIM (same type, same derives, same error type — see
/// `overdrive_core::cgroup` for the definition) so every existing
/// `overdrive_worker::cgroup_manager::CgroupPath` call site keeps
/// resolving unchanged.
pub use overdrive_core::cgroup::{CgroupPath, CgroupPathError};
...
// ---------------------------------------------------------------------------
// Unit tests — pure-logic helpers (cpu_weight_for arithmetic, EBUSY
// discrimination). `CgroupPath` coverage relocated to
// `overdrive_core::cgroup` alongside the type (ADR-0082 §D2 gap 1).
// FS-touching coverage lives in `tests/acceptance/cgroup_manager/`
// against `SimCgroupFs`.
// ---------------------------------------------------------------------------
