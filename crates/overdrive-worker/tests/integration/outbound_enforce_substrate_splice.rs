...
const CIDR_LEASE_NAME: &str = "worker-outbound-enforce-substrate-splice";
...
async fn outbound_enforce_substrate_bidirectional_splice_zero_copy() {
    ...
    clean_shared_infra();
    let lease = TestCidrLease::acquire(CIDR_LEASE_NAME)
        .expect("acquire outbound substrate topology CIDR lease");
