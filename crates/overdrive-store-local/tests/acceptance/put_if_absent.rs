...
async fn put_if_absent_on_populated_key_returns_existing_bytes_and_does_not_overwrite() {
    ...
    match outcome {
        ...
        PutOutcome::Inserted => panic!("expected KeyExists on populated key; got Inserted"),
    }
...
async fn put_if_absent_does_not_advance_commit_index_on_key_exists() {
    ...
    // Every no-op put_if_absent must leave the counter alone.
    for _ in 0..5 {
        let outcome =
            store.put_if_absent(key, b"would-have-clobbered").await.expect("put_if_absent");
        assert!(
            matches!(outcome, PutOutcome::KeyExists { .. }),
            "every attempt against a populated key must return KeyExists",
        );
    }
    assert_eq!(
        store.commit_index(),
...
async fn concurrent_put_if_absent_same_key_different_values_yields_exactly_one_winner() {
    ...
    while let Some(res) = set.join_next().await {
        ...
        match outcome {
            ...
            PutOutcome::KeyExists { existing } => losers_saw_incumbent.push(existing),
        }
    }
