...
impl VmConfig {
    ...
    /// The rootfs clone is a real file, so its byte length is a genuine
    /// `RLIMIT_FSIZE` floor. The guest-RAM term is retained
    /// **conservatively** — a deliberately loose cap, not a tight one:
    /// guest RAM is anonymous memory (not a file) in this feature, so it
    /// does not itself consume the `RLIMIT_FSIZE` budget today. The term
    /// is kept as headroom against a future in which guest RAM is backed
    /// by a memfd (a memfd is a *file* for `RLIMIT_FSIZE`) — which lands
    /// with the DEFERRED managed-volume / `--memory shared=on` work
    /// (GH #97), not in this feature.
    ///
