...
use overdrive_reconcilers::{
    RESTART_BACKOFF_DURATION, WorkloadLifecycle, WorkloadLifecycleState, WorkloadLifecycleView,
};
...
fn recomputes_deadline_at_window_boundary() {
    ...
    assert_eq!(
        ...
        3,
        "tick.now_unix == seen_at + backoff_for_attempt(restart_count) must \
         emit RestartAllocation + service-lifecycle EnqueueEvaluation per GAP-9 + \
...
fn restart_survival_idempotence() {
    ...
    assert_eq!(
        actions_a.len(),
        3,
        "test fixture must exercise the RestartAllocation path (now paired with service-lifecycle EnqueueEvaluation per GAP-9 + svid-lifecycle EnqueueEvaluation per ADR-0067 D5b); got {actions_a:?}",
    );
