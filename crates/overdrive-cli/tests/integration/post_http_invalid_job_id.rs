...
//! response carries a `workload_id` field that the validating
...
//!
//! The `WorkloadId::new(&resp.workload_id)` call site is post-HTTP. The variant was
...
//! `workload_id` JSON body — ≥100 lines of harness for one assertion. The bug
...
/// fails [`WorkloadId::new`](overdrive_core::id::WorkloadId::new) — empty string is
/// rejected by `validate_label` with `IdParseError::Empty { kind: "WorkloadId" }`.
...
fn post_http_invalid_job_id_maps_to_body_decode_not_invalid_spec() {
    ...
    assert!(
        ...
        "post-HTTP `WorkloadId::new` failure must NOT map to CliError::InvalidSpec \
    ...
    assert!(
        matches!(&err, CliError::BodyDecode { cause } if cause.contains("server returned invalid workload_id")),
        "post-HTTP `WorkloadId::new` failure must map to CliError::BodyDecode \
