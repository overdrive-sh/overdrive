...
//!     │
//!     ▼
//! handlers::submit_workload                (Service::from_submit)
//!     │ archive_for_store
//!     ▼
...
//! `WorkloadIntent::Service(Service)` at `workloads/dns-resolver`
...
/// server writes through at `<data_dir>/intent.redb`.
...
async fn deploy_udp_service_is_accepted_and_persisted_intent_carries_proto_udp() {
    ...
    // deserialise the `WorkloadIntent::Service(Service)` at
