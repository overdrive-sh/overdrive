...
/// Step 08-02 GREEN handed off: the `HydratorEventuallyConverges`
/// scaffold is gone, so the downstream-fallout `#[should_panic]`
...
proptest! {
    ...
    /// Step 08-02 GREEN handed off: the `HydratorEventuallyConverges`
    /// scaffold is gone, so the downstream-fallout `#[should_panic]`
    /// attribute is removed per `.claude/rules/testing.md` §
