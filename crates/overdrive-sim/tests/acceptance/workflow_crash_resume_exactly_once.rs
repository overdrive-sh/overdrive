...
//! them drives `WorkflowCtx::run` once via the engine's own
...
async fn killing_after_step_records_does_not_repeat_the_effect_on_resume() {
    ...
    // ---- Run B: crash — drive ctx.run once (records step 0), drop the
    //      future BEFORE terminal. A process-local kill modelled honestly:
    //      the journal carries the RunResult but NO Terminal entry. ----
    ...
    // additional times (replay short-circuited the recorded run). Across
    ...
    assert_eq!(
        ...
        recorded_run(&uninterrupted_journal),
        "the replayed RunResult is byte-equal to the uninterrupted run's recorded step"
    );
}
