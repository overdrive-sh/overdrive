...
use overdrive_core::traits::driver::{
    ...
    DriverStartClass, DriverStartCleanupError, DriverStartFailure, DriverType, VmStartFailure,
};
...
pub(crate) async fn apply_live_mtls_intercepts(
    ...
    for LiveMtlsRecoveryPlan { row, spec } in plans {
        ...
        .await?;
        worker.start_alloc(&spec).await.map_err(|source| ShimError::MtlsRestartInstall {
            alloc_id: row.alloc_id.clone(),
            source,
        })?;
        tracing::info!(
...
/// Put every validated survivor behind a kernel fail-closed boundary before
    ...
    let quarantines = plans
        ...
        .map(|plan| {
            overdrive_worker::mtls_intercept::install_recovery_quarantine(
                plan.spec.host_veth.as_deref(),
                plan.spec.workload_addr,
                &plan.spec.service_ports,
            )
            .map_err(|source| ShimError::MtlsRestartQuarantine {
                alloc_id: plan.row.alloc_id.clone(),
                source,
            })
        })
    ...
    match action {
        ...
        Action::StartAllocation { alloc_id, workload_id, node_id, mut spec, kind } => {
            ...
            if intercept_required {
                ...
                if let Some(worker) = mtls_worker
                    && let Err(cause) = worker.start_alloc(&spec).await
                {
                    teardown_and_release_netns(&alloc_id, net_slot_allocator, network_provisioner)?;
                    let reason = TransitionReason::MtlsInterceptInstallFailed {
                        stage: cause.stage().to_owned(),
            ...
            let start_outcome = match start_outcome {
                Ok(handle) => Ok(handle),
                Err(error) => {
                    let release_network = error
                        .start_cleanup_failure()
                        .is_none_or(DriverStartCleanupError::recovery_complete);
                    Err(rollback_prestarted_allocation(
                        error,
                        &alloc_id,
                        mtls_worker,
                        intercept_required,
        ...
        Action::RestartAllocation { alloc_id, mut spec, kind } => {
            ...
            if cleanup_retry_driver.is_none()
                && matches!(
                    &start_outcome,
                    Err(DriverError::StartRejected { failure })
                        if is_duplicate_vm_owner(failure, &alloc_id)
            ...
            let start_outcome = if cleanup_retry_driver.is_none() {
                match start_outcome {
                    Ok(handle) => Ok(handle),
                    Err(error) => {
                        let release_network = error
...
pub enum ShimError {
    ...
    /// A surviving workload could not be placed behind the temporary
    /// fail-closed kernel boundary required before dead-rule sweeping.
    #[error("mTLS restart quarantine install failed for {alloc_id}: {source}")]
    MtlsRestartQuarantine {
        alloc_id: AllocationId,
        #[source]
        source: overdrive_worker::mtls_intercept::InterceptError,
    },

    /// The atomic handoff from temporary DROP rules to the complete rebuilt
    /// intercept batch failed. The transaction rolls back and leaves every
    /// survivor quarantined for the next boot attempt.
    #[error("mTLS restart quarantine release failed: {0}")]
    MtlsRestartQuarantineRelease(overdrive_worker::mtls_intercept::InterceptError),
