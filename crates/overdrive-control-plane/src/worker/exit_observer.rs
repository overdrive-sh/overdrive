...
/// `Some(state.runtime.clone())`) but is now unused by the body;
/// collapsing [`spawn`] and this entry-point into one is a follow-up
/// that touches the composition root and is out of the single-cut's
/// scope.
///
/// # Shutdown
///
...
pub fn spawn_with_runtime(
    ...
    events: Arc<broadcast::Sender<LifecycleEvent>>,
    clock: Arc<dyn Clock>,
    _runtime: Option<Arc<ReconcilerRuntime>>,
