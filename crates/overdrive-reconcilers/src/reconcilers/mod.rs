//! First-party reconciler IMPLS + enum dispatch (ADR-0086 D2/D3).
//!
//! Extracted out of `overdrive-core` per ADR-0086: the reconciler CONTRACT
//! (the [`Reconciler`] trait, [`Action`], [`TickContext`], [`ReconcilerName`],
//! [`TargetResource`], the resync/interest vocabulary, `HydrationContext` /
//! `HydrateError`) STAYS in `overdrive_core::reconcilers`; the impls, the three
//! dispatch enums ([`AnyReconciler`] / [`AnyState`] / [`AnyReconcilerView`]),
//! the per-reconciler `State`/`View` projections, and the pure helpers move
//! here. This crate depends only DOWN on `overdrive-core` — the edge that
//! breaks the Cargo cycle (ADR-0086 D4).
//!
//! The contract vocabulary is re-imported (privately) from
//! `overdrive_core::reconcilers` below so the impl submodules keep referencing
//! it via `super::` exactly as they did pre-move — a pure mechanical
//! extraction with no behaviour change (step 02-02).
...
// `ServiceLifecycleReconciler` lives in `crate::service_lifecycle` (the moved
// module, a sibling of this one) for the cycle-breaking reasons documented at
// that module's header. Re-import here so the dispatch enums can reference it
// without forcing every dispatcher to spell the full path.
