...
| [E07](E07-vm-job-calls-exec-service/) | E | one VM Job calls one VM Service and receives the expected reply through the built default-feature product | Q9 | S-GTI-01, DESIGN Q9, ADR-0088, ADR-0089 | `pending` |
...
| [E10](E10-vm-service-http-cross-driver-status/) | E | historical pre-cut Exec/VM comparison expectation | K2 | S-SVM-26, US-SVM-2, ADR-0032/0059/0078/0090/0093/0099/0100, GH #257 | `out-of-scope — pre-cut cross-driver artifact; historical evidence retained` |
...
- **public-ingress-gateway** — E14 is the point-in-time operator receipt for
  the publicly trusted Web-PKI journey. The recurring AT-PIG-E2E-1 Tier-3
  runner owns kernel, wire, selected-identity and cleanup regression evidence;
  E14 captures only built-product commands, redacted status, TLS identity and
  the stakeholder-visible streamed response.
...
- **service-kind-vm-workloads** (GH #257) — E08 is the sole black-box walking
  ...
  <SPEC>` path on native metal and observe only CLI, workload traffic, and
  cleanup surfaces. Service traffic is generated by seven checked-in plaintext
  `[job] + [vm]` clients resolving `*.svc.overdrive.local`; the bundle's one
  E07-style `prepare.sh` installs their static client binary in its
  marker-owned private rootfs. Those binaries hold no SVID and succeed only on
