...
# O05 — issued_certificates audit row is observable via `workload describe`.
#
# Needs the built-in CA issuance path (DELIVER): SVID issuance on alloc-start
# writes the issued_certificates observation row, surfaced by `workload describe`.
...
capture alloc_status od workload describe "${job_name:-coinflip}" || true

# Sub-claim 1 — workload describe surfaces the live `Issued certificates:` section
...
if grep -qE '^Issued certificates:' "$status_out" \
  ...
  echo "  [PASS] workload describe surfaces 'Issued certificates:' with serial / spiffe_id / issuer_serial / not_after"
  ...
  if grep -qiE 'BEGIN (CERTIFICATE|PRIVATE KEY|EC PRIVATE KEY|RSA PRIVATE KEY)' "$status_out"; then
    echo "  [FAIL] workload describe leaks PEM cert/key bytes — the summary must be metadata-only"
    rc=1
  else
    echo "  [PASS] no cert bytes / no private key in the render (metadata-only, as designed)"
  fi
else
  echo "  [pending] workload describe shows no 'Issued certificates:' section yet — the"
