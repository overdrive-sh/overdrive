...
Overdrive architecture, subsystem, and process patterns — the reconciler and
workflow contracts, the state-layer and port-trait boundaries, rkyv persistence,
xtask discipline, aya-rs kernel-side patterns, and the commit / review /
deletion workflow. These are not architectural decisions (those live in the
whitepaper); they are the conventions for how the codebase is wired together.
