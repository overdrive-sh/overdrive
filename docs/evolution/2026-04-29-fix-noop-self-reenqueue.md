...
- **Workspace gates green**: `cargo nextest run --workspace` (510 tests), `cargo test --doc -p overdrive-control-plane`, `cargo nextest run --workspace --features integration-tests --no-run` (macOS typecheck), `cargo dst` (14 invariants), `cargo xtask dst-lint`, `cargo clippy -p overdrive-control-plane --all-targets -- -D warnings`.
