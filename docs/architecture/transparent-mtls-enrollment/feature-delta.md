...
**Path A** (the spike's ratified direction). It does NOT design the
expected-SVID/intended-peer join (#242) or the **node-local name responder**
(#243, an in-agent name-answering listener — NOT a separate daemon) — it defines
the **boundary contract** with them and consumes them per-connection.
...
fail-closed; Q4 = both directions, intended-peer pinning deferred to #242). Q5a
**chosen**: the DNS name-layer **integration** is folded into THIS design (the
node-local name responder #243 and the VIP allocator remain separate builds —
only the integration shape lives here). See § "Name-layer integration (Q5a)" and the
...
| research `…stable-service-naming-and-transparent-mtls…` | ✓ | name/resolve/enforce layering; `service_backends` (BUILT) data source; the `{(backend_addr, expected_svid)}`-filtered-to-`running` resolve contract (expected-SVID join #242, NOT built); `expected_peer`/`PeerIdentityMismatch` reserved. |
...
(`service_backends`, expected-SVID join #242) and the name context (#243) are
**separate, named dependencies** (anti-corruption boundary = the resolve port).
The node-local **name responder** (#243 — an in-agent name-answering listener
sharing the `ServiceBackendsResolve` index, NOT a separate daemon) and the VIP
allocator (#167) remain separate builds; only the *integration* of a node-local
responder into the per-workload netns (the injection mechanism + the return
shape + its composition with `MtlsResolve`) lives in this feature.
...
| **resolv.conf injection (name-layer integration, Q5a)** | per-alloc netns provisioner (same home as netns+veth, Q2 → `veth_provisioner` EXTEND) | EXTEND | When creating the per-alloc netns, write the node-local name responder address into the netns's `/etc/resolv.conf` (per-netns mount, the standard `ip netns` convention) so the workload's libc `getaddrinfo` reaches the responder with no app config (Fly.io `fdaa::3` model). The **responder itself is #243** — an in-agent name-answering listener sharing the `ServiceBackendsResolve` index, NOT a separate daemon — this feature wires only the injection. |
| **DNS-return contract alignment (name-layer integration, Q5a)** | design-only; the contract `MtlsResolve` recognizes | n/a (contract, not code) | The responder returns a `running` backend addr from `service_backends` (**headless**, v1) — that returned address IS the `orig_dst` `MtlsResolve.resolve(orig_dst)` later recognizes. This feature OWNS keeping the two contracts consistent; it does NOT own the responder's query path (#243) nor a VIP allocator (#167). |
...
identify`/veth egress; DNS #243-gated, excluded). Full pinned shapes:
...
#241's inbound nft-TPROXY install will consume) — NOT a test-only hook. v1 uses it
as the Tier-3 inbound test-observability seam: with the inbound redirect
#241-deferred, a Tier-3 test installs its own redirect to leg-C and drives the
spawned production inbound `accept_loop`. It exposes ONLY a socket address — no
SVID/identity material — so the authn-only v1 boundary (D-TME-8 / #242) is
...
`design/wave-decisions.md` § "D-TME-13". Cites ADR-0071 + #241.
...
| **`MtlsResolve` (NEW)** | CREATE-NEW (this feature defines the contract) | `ServiceBackendsResolve` (reads `service_backends` via `ObservationStore`) — v1 SHELL; the expected-SVID join is #242 | `SimMtlsResolve` (scriptable `orig_dst → MtlsResolution`) | `resolve(orig_dst: SocketAddrV4) -> Result<MtlsResolution>` — a **3-variant sum type** return, NOT a binary `Option` (see § "`MtlsResolve` port contract" below). Filtered to `running`. The #242 expected-SVID-join boundary: THIS feature owns the port + the `service_backends`-reading shell; #242 owns the expected-SVID join and the multi-backend LB-pick policy. |
...
- **v1 `ServiceBackendsResolve` returns `expected_svid: None`** for every
  ...
  #242 anti-corruption boundary — explicitly forbidden (consistent with Q4: v1
  is authn-only, intended-peer pinning deferred to #242).
...
- **Classification onto the index** (consistent with C1 and the shipped 01-01
  ...
  - `orig_dst` **hits** a `running` mesh backend in the index →
    ...
    `None` for every backend in v1 — the identity join is #242; C2).
...
1. **Cilium is the canonical implementation of exactly this pattern.** Its
   ...
   **relists on `ErrCompacted` or any watch error** (`goto reList`) with stale
   ...
   deferral of the identity join to #242; and it fail-CLOSES an auth-required
...
3. `…stable-service-naming-and-transparent-mtls-comprehensive-research.md`
   ...
   shape (addr→backend, then backend→identity at #242).
...
| D-TME-6 | A new `MtlsResolve` driven port is the #242 expected-SVID-join anti-corruption boundary; this feature defines the contract + a v1 `service_backends`-reading host adapter; **fail-closed (not silent)** at the boundary; #242 owns the expected-SVID join, #243 owns the name layer (the in-agent name responder). *(REVISED 2026-06-22: #178 split — the resolve/expected-SVID-join boundary is now #242, the name responder is now #243; see #178 supersession.)* | SETTLED (Q3 ratified) |
...
| D-TME-8 | v1 scope = **BOTH directions**; intended-peer SVID pinning (`expected_peer`/`PeerIdentityMismatch`) **deferred to #242** (v1 = authn-only) (Q4 ratified). *(REVISED 2026-06-22: deferral target #178 → #242 per the #178 split.)* | SETTLED (Q4 ratified) |
| D-TME-9 | **Name-layer integration (Q5a)**: a node-local name responder is injected into the per-workload netns `resolv.conf` (Fly.io `fdaa::3` model); the responder is **#243** — an **in-agent name-answering listener** sharing the `ServiceBackendsResolve` index (NOT a separate daemon, NOT in-kernel), only the injection + return-shape contract live here. *(REVISED 2026-06-22: #178 split — the responder is now #243 and reframed daemon → in-agent listener; injection (D-TME-9) + headless return shape (D-TME-10) substance UNCHANGED.)* | SETTLED (Q5a folded in) |
...
| **`MtlsResolve` port** | **CREATE-NEW (justified)** | No existing port returns `orig_dst → {backend_addr, expected_svid}` filtered to `running`. The closest surfaces are `Dataplane` (map writes, wrong shape) and the `service_backends` table (a data source consumed only by the LB hydrator, no client-facing resolve call). The enrollment model REQUIRES a per-connection resolve consumer; no existing port fits. The port is the #242 expected-SVID-join anti-corruption boundary (the research's R1/R2 named gap). Extending `MtlsEnforcement` is rejected — it would entangle the resolve concern into the enforcement contract the orchestrator explicitly froze at 4 methods. |
...
| **resolv.conf injection (name-layer, Q5a)** | **EXTEND** | The per-alloc netns provisioner (Q2 = extend `veth_provisioner`) already owns netns creation; writing the responder address into the netns's `/etc/resolv.conf` is one more idempotent converge step in the same converge-on-boot shape (a per-netns mount, the stock `ip netns exec` convention). No new component — it is a parameter + a step on the provisioner. The responder itself (#243 — an in-agent name-answering listener, NOT a separate daemon) is a separate build, NOT created here. |
| **Node-local name responder** | **DEPENDENCY (NOT built here, #243)** | The **in-agent name-answering listener** that answers `<job>.svc.overdrive.local` reads the SAME in-RAM `ServiceBackendsResolve` index the outbound resolve + inbound install already hold (one source, three readers) — NOT a separate daemon, NOT in-kernel. It is the #243 name-layer build. This feature consumes its *address* (for injection) and aligns its *return shape* (headless) with `MtlsResolve` — it does NOT build the responder. Challenged CREATE-NEW: building a responder here would duplicate #243 and cross the scope boundary the dispatch fixes. |
...
resolv.conf injection is an EXTEND of the netns provisioner, and the node-local
name responder (#243 — an in-agent name-answering listener) + VIP allocator
(#167) are named DEPENDENCIES, not builds in this feature. Every other change is
EXTEND or DELETE.
...
heavy #243 responder implementation). Path A's per-workload netns (Q2) is the
injection point; the name layer's job is to give an unmodified workload a stable
name to dial that lands at an address `MtlsResolve` recognizes.
...
Q2 provisioner). **#243 owns** the name responder — an **in-agent name-answering
listener** sharing the `ServiceBackendsResolve` index (NOT a separate daemon,
NOT in-kernel), answering `<job>.svc.overdrive.local` from that same in-RAM
index (one source, three readers: outbound resolve + inbound install + name
answers; see #243 for the full shape). The boundary is the responder's *address*
...
|---|---|---|
| DNS returns | A `running` backend addr straight from `service_backends` (K8s-headless / Fly.io endpoint-set / #243-shaped — research §2.5, §2.7) | A per-service stable virt address (`<job>.svc.overdrive.local → fdc2:…::N`, K8s-ClusterIP / Consul-`virtual.consul` shape — research §2.5) |
...
| Keeps `MtlsResolve` v1 thin | YES — v1 resolve = identity join, NOT LB (and LB-pick is the **#242-deferred** multi-backend policy, D-TME-8) | NO — v1 resolve would have to own the VIP→backend LB-pick this feature explicitly defers |
...
2. **Keeps `MtlsResolve` v1 honest with the deferred-LB boundary.** v1 resolve
   ...
   `expected_svid = None` until #242 — D-TME-8). A VIP return would force the
   resolve port to own a VIP→backend LB-pick, which is the multi-backend policy
   D-TME-8 explicitly defers to #242. Headless avoids the contradiction.
...
workload getaddrinfo("payments.svc.overdrive.local")
  → resolv.conf (injected into the per-workload netns) points at the
    node-local name responder (#243 — in-agent name responder)
  ...
  → enforce Routed::Outbound { peer: B }: rustls client handshake to B presenting workload SVID,
    chain-to-bundle verify (+ SAN==expected_svid once #242 lands), kTLS arm, agent-light pumps
...
| **Q3** — `MtlsResolve` contract + #242 boundary? | **Port in `overdrive-core` + a v1 `service_backends`-reading host adapter**; **fail-closed (not silent)** at the boundary (a resolve adapter that cannot read the store refuses boot — it does NOT silently return empty, which would re-introduce the silent-cleartext footgun). Inlining the read in the worker REJECTED (DST-uninjectable; hard-couples enforce to the observation schema). **#242 owns** the expected-SVID join + multi-backend LB-pick + the SAN-match wiring of `expected_peer`; **#243 owns** the name→virt layer upstream of `orig_dst` (the in-agent name responder). | D-TME-6 |
| **Q4** — v1 scope; intended-peer pinning? | **BOTH directions in v1** (Path A's point is symmetry on one mechanism; the inbound nft-TPROXY install is the proven template the outbound mirrors). Intended-peer SVID pinning (`expected_peer`/`PeerIdentityMismatch`) **DEFERRED to #242** — v1 stays authn-only (chain-to-bundle); the resolve port carries `expected_svid` so the pin wires the moment #242 supplies the join. Docs/tests MUST NOT call the wrong-but-valid-peer case "protected" until #242. | D-TME-8 |
...
- **(a) Headless (RECOMMENDED)** — the node-local responder returns a `running`
  ...
  **No new v1 dependency** (no VIP allocator); DNS and `MtlsResolve` read the
  ...
  Keeps `MtlsResolve` v1 thin and honest with the #242-deferred LB boundary.
...
targets verify against live code. Q5a correctly defers the #243 name responder
(an in-agent name-answering listener) and #167 VIP allocator to separate builds
while folding only the resolv.conf injection + the headless v1 return-shape into
scope. The single open item
...
cites a real issue (#241, #242, #243, #244, #167, #197, #234).
...
- **None.** The enrollment model (per-connection resolve, fail-closed on miss)
  ...
  bundle, intended-peer pinning deferred to #242) is documented and tests are
  flagged to NOT call the wrong-but-valid-peer case "protected" until #242.
...
2. **Bound the `ResolvedBackend` shape to exactly `{addr, expected_svid}`.** Note
   explicitly that multi-backend candidate sets + LB-pick are #242's concern, so
...
1. **Forward-compatibility of headless → VIP (D-TME-10).** The "additive VIP arm,
   ...
   When #167/#242 land, a design-amendment must pin the disambiguation mechanism
...
- Anti-corruption boundaries are enforced by discipline, not the compiler:
  ...
  #242), so a crafter is not tempted to thread `IdentityRead` and fill
...
- **Suggestion 2 (C2) — PINNED.** `ResolvedBackend` is bounded to exactly
  ...
  `expected_svid: None` (authn-only; the expected-SVID join is #242; filling it
