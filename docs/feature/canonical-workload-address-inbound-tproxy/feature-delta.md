...
| Egress resolve index key | `by_addr: BTreeMap<SocketAddrV4, BTreeMap<ServiceId, Backend>>` — ownership-aware, keyed on `Backend.addr` (B2's reader) | `mtls_resolve_adapter.rs:214` |
...
| VIP-dial path / VIP allocator / multi-node | Out of scope (no live VIP consumer; TEACH not needed until this ships) | **#61 / #167** |
