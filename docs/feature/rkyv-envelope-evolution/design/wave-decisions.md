...
### Typed codec module (UI-03)

**Amended 2026-05-12 (UI-03 reconciliation).** The `IntentStore`
trait is correctly a **generic byte-level k/v store** (it persists
`Job` aggregates, `WorkloadKind` discriminator bytes per ADR-0047,
stop sentinel markers, and frame-wrapped snapshot bytes per
ADR-0020; the trait is also shared with the future `RaftStore`
Phase 2 path whose snapshot contract relies on byte identity).
The trait surface MUST NOT be refactored to take a typed `Job` on
write/read methods.

Wrapping discipline for `Job` lives in a **Job-specific codec
module on the `Job` type itself**, NOT on the `IntentStore`
trait. The codec module provides three methods co-located with the
`Job` aggregate:

- `Job::archive_for_store(&self) -> Result<AlignedVec, EnvelopeError>`
  — wraps via `JobEnvelope::latest(self.clone())` and rkyv-encodes.
- `Job::from_store_bytes(bytes: &[u8], redb_path: &Path) ->
  Result<Job, IntentStoreError>` — rkyv-deserialises as
  `JobEnvelope`, projects via `into_latest()`, emits
  `health.startup.refused` on `EnvelopeError` before returning the
  error.
- `Job::spec_digest(&self) -> Result<ContentHash, EnvelopeError>` —
  SHA-256 over `archive_for_store` bytes. Operator-observable Job
  ID change (envelope bytes, not raw V1 bytes); greenfield single-
  cut absorbs.

Every Job writer migrates `rkyv::to_bytes(&job)` to
`job.archive_for_store()?`. Every reader migrates inline
`rkyv::access::<ArchivedJob>` to `Job::from_store_bytes(bytes,
&path)?`. `LocalStore::open`'s recovery walk calls
`Job::from_store_bytes` per `jobs/`-prefixed entry. Non-Job byte
values continue to use the `IntentStore` byte-level surface
directly. See amended ADR-0048 § "Intent persistence boundary —
typed codec on `Job`" for the full rationale and the rejected
alternatives (typed-trait refactor; typed helper methods with
default impl).
...
- **(C) Write invariant**: two complementary layers per honest mechanism statement (revised 2026-05-12 per peer review; **amended 2026-05-12 UI-01 reconciliation — rustc E0446**; **further reconciled 2026-05-12 UI-02 — alias-to-payload public API**).
  ...
  - **UI-03 (2026-05-12 DELIVER) — typed codec module on `Job`; trait unchanged.** Surfaced during step 01-04 when migrating the LocalStore intent read/write paths. The `IntentStore` trait was found to be a generic byte-level k/v store (persists Jobs, kind discriminators, stop markers, snapshot frames; shared with future `RaftStore` Phase 2 snapshot contract). The trait surface is correctly bytes-passthrough and MUST NOT be refactored. Decision: shape **B(i)** — codec module on `Job` (`Job::archive_for_store` / `Job::from_store_bytes` / `Job::spec_digest`); trait surface unchanged. `spec_digest` is now SHA-256 of envelope bytes (operator-observable Job ID change, greenfield single-cut absorbs). Rejected: shape **A** — refactor `IntentStore` trait to take typed `Job` (breaks non-Job value classes + RaftStore snapshot contract). Rejected: shape **B(ii)** — typed `put_job` / `get_job` helper methods with default impl on the trait (trait surface bloat for the convenience of a single value class; per-implementation coordination cost). See amended ADR-0048 § "Intent persistence boundary — typed codec on `Job`" and `docs/feature/rkyv-envelope-evolution/deliver/upstream-issues.md` (UI-03).
...
| UI-03 (2026-05-12 DELIVER) — typed codec module on `Job`; trait unchanged | Blocking (DELIVER-surfaced) | Surfaced during step 01-04 when migrating LocalStore intent read/write paths to the envelope. The `IntentStore` trait was found to be a generic byte-level k/v store (persists `Job` aggregates, `WorkloadKind` discriminator bytes per ADR-0047, stop sentinel markers, frame-wrapped snapshot bytes per ADR-0020) and is shared with the future `RaftStore` Phase 2 path whose snapshot contract relies on byte identity. The trait surface MUST NOT be refactored to take a typed `Job` on write/read methods. **Decision (2026-05-12, user-approved)**: shape **B(i)** — codec module on `Job` (`Job::archive_for_store` / `Job::from_store_bytes` / `Job::spec_digest`); trait surface unchanged. `spec_digest` is option **(a)** — SHA-256 over envelope bytes, not raw V1 bytes; operator-observable Job ID change absorbed by greenfield single-cut per ADR-0048 § 5. The "intent persistence boundary" conceptually moves from `LocalStore::open` to the `Job` codec module — `LocalStore::open`'s recovery walk calls `Job::from_store_bytes` per `jobs/` entry to surface envelope-decode errors at boot. Rejected: shape **A** — refactor `IntentStore` trait to take typed `Job` (breaks non-Job value classes + RaftStore snapshot contract; Phase-1-out-of-scope trait redesign). Rejected: shape **B(ii)** — typed `put_job` / `get_job` helper methods with default impl on the trait (trait surface bloat for the convenience of a single value class; per-implementation coordination cost). Originating record: `docs/feature/rkyv-envelope-evolution/deliver/upstream-issues.md` (UI-03). | ADR-0048 § 4b "Intent persistence boundary — typed codec on `Job`", Alternatives → "Option 7 — Refactor `IntentStore` trait to take typed `Job`", "Option 8 — Add typed `put_job` / `get_job` helper methods on the trait with default impl"; this file § H "Typed codec module (UI-03)", § 9 (C) UI-03 sub-bullet; `.claude/rules/development.md` § "rkyv schema evolution" → "Typed persistence-boundary codec" callout; `docs/feature/rkyv-envelope-evolution/distill/test-scenarios.md` S-EV-03 driving-port update; `docs/feature/rkyv-envelope-evolution/distill/red-scaffolds.md` (driving-port phrasing if present); `docs/feature/rkyv-envelope-evolution/deliver/roadmap.json` step 01-04 criteria + implementation_notes + files_to_modify + effort_hours; `docs/feature/rkyv-envelope-evolution/deliver/upstream-issues.md` UI-03 entry |
