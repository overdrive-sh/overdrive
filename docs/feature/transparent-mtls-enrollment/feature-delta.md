...
| **Workload `NetSlot`** | A host-unique bounded per-allocation network slot (`NetSlot(u16)`, `0..=4095`). The single axis from which the veth iface names AND the point-to-point /30 subnet derive — collision-free by construction, NOT a hash of the `AllocationId` (pigeonhole — a 253-char id cannot collision-free-map into a 15-char iface name). Assigned smallest-free at the C3 `on_alloc_running` hook, released at `on_alloc_terminal`. The netns name keeps the readable `ovd-ns-<alloc>` form (a `/var/run/netns/` filename, not IFNAMSIZ-bound). See D-TME-12. |
