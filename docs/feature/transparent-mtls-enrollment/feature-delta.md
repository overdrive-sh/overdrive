...
| **Workload veth pair** | `(ovd-wl-<alloc>` in-netns ↔ `ovd-hv-<alloc>` host-side). The host-side end is where nft-TPROXY PREROUTING intercepts BOTH the workload's egress (now ingressing the host veth) and inbound traffic to the workload. |
...
**CREATE-NEW count: 1** (`MtlsResolve` port). The name-layer integration (Q5a)
adds **zero** new CREATE-NEW: resolv.conf injection is an EXTEND of the netns
provisioner, and the DNS responder daemon (#61) + VIP allocator (#167) are
named DEPENDENCIES, not builds in this feature. Every other change is EXTEND or
DELETE.
