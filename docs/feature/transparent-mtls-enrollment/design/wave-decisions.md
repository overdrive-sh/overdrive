...
| D-TME-11 | **Resolve READ MECHANISM (C4; refines D-TME-6)**: `ServiceBackendsResolve` resolves `orig_dst` against an **in-RAM, address-keyed reverse index** of the `running` `service_backends` set (`addr → Backend`), built/refreshed from the EXISTING `ObservationStore` `subscribe_all()` / bulk-load-then-observe surface — NOT a per-`ServiceId` point query (the `ServiceId`-keyed `service_backends_rows` is the wrong surface; the adapter holds no `ServiceId`), and MUST NOT add a new `ObservationStore` trait method. **A miss = `NonMesh`** (cleartext pass-through), NOT `MeshUnreachable`; the un-converged-miss / fail-toward-handshake hardening is multi-node, tracked in **#236**. The v1 adapter realizes this as a **forward-only `subscribe_all()` index with no `service_backends` bulk-load** — so a control-plane restart starts the index empty and opens a `NonMesh`-cleartext window until `BackendDiscoveryBridge` re-converges; this restart-window instance is **v1-accepted, tracked in #237** (04-02 composition root MUST subscribe-before-first-write). PUBLIC `MtlsResolve` API unchanged (adapter-internal detail). | D-TME-6 pinned the resolve *model* but not the *read mechanism*; a DELIVER step surfaced that `resolve(orig_dst)` has no `ServiceId` and no addr→service surface exists. Cilium's `ipcache` (in-RAM addr→identity reverse index, subscribe-populated) is the canonical precedent; research §4.1 already describes the resolve as an in-RAM `service_backends` lookup. Making a miss fail-closed would break legitimate `NonMesh` external egress — forbidden. (Ratified 2026-06-16.) |
...
| **C4** (added 2026-06-16, post-DESIGN amendment) | `ServiceBackendsResolve` resolves `orig_dst` against an **in-RAM, address-keyed reverse index** of the `running` `service_backends` set (`addr → Backend`), built/refreshed from the EXISTING `ObservationStore` `subscribe_all()` surface — NOT a per-`ServiceId` point query, and **MUST NOT add a new `ObservationStore` trait method**. Miss-classification scoping: a **miss = `NonMesh`** (cleartext pass-through, by design), NOT `MeshUnreachable`; the bounded cleartext edge is closed by the headless single-source invariant (D-TME-10); fail-toward-handshake on an un-converged miss is multi-node hardening (**#236**). **PUBLIC API unchanged** (adapter-internal). Pins the *read mechanism* D-TME-6 left underspecified (surfaced by a DELIVER step). | feature-delta § "`MtlsResolve` port contract" (C4) + D-TME-11 row; ADR-0071 § "The new driven port" (C4); this file (D-TME-11). Consistent with the shipped 01-01 port rustdoc (`crates/overdrive-core/src/traits/mtls_resolve.rs`) — ADDS the read mechanism, does NOT re-classify. |
