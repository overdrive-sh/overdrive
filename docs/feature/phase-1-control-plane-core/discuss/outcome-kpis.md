...
| K4 | Overdrive platform engineer | Writes a reconciler and has the DST harness prove `reconciler_is_pure` + `duplicate_evaluations_collapse` | Both DST invariants pass on every run; `at_least_one_reconciler_registered` always holds at boot | N/A (greenfield — no reconciler primitive exists today) | `overdrive-sim` invariant catalogue adds three new invariants; `cargo dst` gates on them | Leading — primary |
...
| K4 | `cargo dst` invariant catalogue | Three new invariants in `overdrive-sim::invariants` + their evaluators | Every PR | CI |
