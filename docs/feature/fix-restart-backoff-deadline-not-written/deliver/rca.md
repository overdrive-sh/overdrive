...
- **DST exposure**: any existing DST scenario that submits a failing alloc and expects rapid restarts within a single second will now see the gate fire. Run `cargo dst` as part of the fix; investigate any scenario that implicitly depended on zero-delay restarts (those were exercising the bug, not the spec).
