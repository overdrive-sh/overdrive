...
| Effort | >2 weeks | **~22–32 d across six slices** (was ~15–23 d across five) — **superseded 2026-08-11 by the D-3 fold-in re-size, now ~27–37 d; see § Scope re-assessment after the D-3 fold-in below** | **Signal fires — resolved by slicing, as before** |
...
### Scope re-assessment after the D-3 fold-in (2026-08-11) — **guardrail 3 fires**

Closing out a deferral this same DESIGN pass left open. Slice 01's Dependencies section
named the cgroup-OOM diagnosis fold-in (`CgroupAccounting` port + two adapters + probe +
the exit-observer/classification work it feeds — ADR-0082 § D8, ADR-0083 § D5 row 13) as
genuinely new, un-costed work and punted the number to DISTILL. Priced here instead, per
user instruction to close it out now rather than leave it for the next reader — the
itemized table lives in `slices/slice-01-vm-job-boots-and-exit-code-is-honest.md` §
Dependencies.

**Slice 01: 5–8 d → 11–14 d.** The addition (6 d) is not folded into the `[D7]` band
above — `[D7]` genuinely cost nothing new (items already inside the slice's own spawn
path). D-3 is a new port, two adapters, a probe, an equivalence test, a new classification
branch and Cause variant, a new Tier-3 AC, and two musl CI targets — none of which existed
in any prior estimate of this slice.

**Re-run against both guardrails, as required at the DESIGN handoff:**

| Guardrail | Threshold | Current | Verdict |
|---|---|---|---|
| 2 — Slice 04's own lift trigger | Slice 04 upper bound > 9 d | 6–9 d, unaffected by this pass (see the `shmem_enabled` row correction in slice-04 — it does not move the headline band) | **Does not fire** (already re-run once this pass at the DESIGN handoff; re-confirmed here since nothing in Slice 01's change touches Slice 04's cost) |
| 3 — wave-unadjustable: >35 d total feature effort, **or** any slice >50% over its stated upper band | Slice 01's stated upper band is 8 d; 50% over is 12 d | Slice 01's re-sized upper bound is **14 d — 75% over the 8 d band, past the 50% ceiling** | **FIRES**, on the single-slice clause alone |

**Total feature effort, recomputed the same way the `~22–32 d` and `~15–23 d` figures
above were computed** (summing each slice's stated lower and upper bounds independently):

| Slice | Band |
|---|---|
| 00 | 2–3 d |
| 01 | **11–14 d** (was 5–8 d) |
| 02 | 2–3 d |
| 03 | 4–6 d |
| 04 | 6–9 d |
| 05 | 2 d |
| **Total** | **27–37 d** (was ~22–32 d) |

The upper-bound reading (37 d) is past the guardrail's 35 d ceiling; the midpoint reading
(≈32 d) is not. The single-slice clause does not depend on which reading is used — it
fires on Slice 01 alone, unambiguously, regardless of how the total is read.

**This is reported, not resolved.** DESIGN is not splitting the feature, re-drawing slice
boundaries, or shrinking Slice 01's scope to duck under 12 d. The trigger is pre-committed
(DISCUSS amendment 2's review fixes, 2026-08-02) precisely to force this decision into the
...
| 2026-08-11 | DESIGN (application — Morgan, user ruling closes the S-VM-67 open item) | **Closes the last open item DISTILL-surfaced gap 2 left standing.** The prior gap-closure entry recorded ADR-0083 § D8's `vmm_override` seam as explicitly NOT reaching S-VM-67 (storage-daemon `--sandbox=namespace` unavailable) and named two honest paths without choosing between them: (a) mint a Slice-04 storage-daemon supervision port mirroring § D8's `Vmm` shape, or (b) assert the fail-closed case at a level narrower than a real `overdrive serve`. **The user has now ruled path (b).** ADR-0083 § D8 is amended (Status header, the closing "what the seam does NOT reach" subsection, and the Consequences → Negative bullet): the `--sandbox=namespace` posture is verified at the **launch-argument construction layer** — the same enforcement tier ADR-0082 § D2.1 already uses for `image_type=raw` (private fields, one rendering site, a pure unit test on the rendered value) — never through a real `overdrive serve`. **The negative decision is recorded explicitly: this feature mints no storage-daemon supervision port.** If Slice 04's virtiofsd lifecycle later needs one on its own merits (supervision, restart, health), that is a decision made then, on those merits, never inherited from this ruling. Stated honestly, both ways: the argv-layer assertion is a complete proof of what argument `VmDriver` constructs, but it does **not** prove a running `virtiofsd` actually enforces the sandbox or fails closed when the host genuinely cannot supply it — that remains a Tier-3 property of Slice 04, undischarged by this ruling. And against `[D8d]`'s own fail-closed-never-silently-downgraded requirement: the silent-downgrade bug is made **lint/test-detected by this ruling, not structurally unrepresentable at the type level** — the same honesty D2.1 already states about its own enforcement tier ("private fields + one rendering site + a lint clause… not a type-level impossibility"). A companion cross-reference correction landed in ADR-0082 (its Status header's now-stale "not resolved here or in ADR-0083" claim, and its § D5 fault-injection table's closing cross-reference). `distill/*` was NOT touched (concurrent DISTILL pass owns it — this ruling unblocks its own S-VM-67 "BLOCKED on a scoping decision" note, but the DISTILL artifact itself is that pass's to update). No Rust exists for this feature yet; nothing here claims otherwise. No GitHub issues created or cited (#259–#263 remain the only real numbers in scope). No commit made. |
...
| 2026-08-11 | DESIGN (domain — Hera, ADR mint) | **Deferral H-1 discharged: DD-1 / DD-1(b) / DD-1(b.i) minted as [ADR-0081](../../product/architecture/adr-0081-three-ending-classes-platform-reclamation-and-artifact-disposal.md).** Extracted the platform-wide Ending Class taxonomy (Intentional Stop / Workload Failure / Platform Reclamation), the DD-1(b) precondition (Platform Reclamation vs Artifact Disposal as a non-ending sibling concept), and the DD-1(b.i) supervision-handle lifecycle (a claim on *authoring an ending*, not a grip on a running process) out of `brief.md` § *Domain Model* into a durable ADR — reserved as ADR-0081 by H-1, now taken. States precisely **which ADR-0078 clause narrows**: `CrashFacts::advance`'s *"unreachable in Phase 1"* edge-case docstring (§ D1) becomes false the moment `StoppedBy::PlatformReclaimed` lands, since `Terminated → Running` on the same LWW key becomes reachable — and reachable *correctly* — for the reclamation-restart path; `advance` itself needs no code change, only its reachability claim is stale, and the DELIVER-wave commit that lands the disposition owes ADR-0078 a dated in-place amendment note (not performed here — editing an accepted ADR is a user decision). States the relationship to ADR-0037: D1's "no reconciler may author a terminal claim on a Platform-Reclamation row" is a corollary of ADR-0037's own definition of `TerminalCondition` ("no further convergence work will be attempted") applied to a class whose run has not concluded — binds every present and future `TerminalCondition`-emitting site, does not narrow the type or its publication boundary. Cross-referenced both ways with the two already-accepted companion ADRs that consumed this decision by name before it had a number: ADR-0082 § D8 (`CgroupAccounting`/`VmOutOfMemory` — checked against this ADR's boundary and correctly excluded, since a cgroup OOM never loses the supervision handle) and ADR-0083 §§ D5–D7 (the binding sites, the two `Action` variants, the `VmReclamation` reconciler and the handle's lifecycle in code). `brief.md` § *Domain Model* gained two pointer notes (the Status-table row and a paragraph ahead of the DD-1 heading) linking to the new ADR; no other section touched. Deferred, unrelated work cited by existing issue number only where mentioned for completeness (#260–#263), none re-litigated. No GitHub issue created — this entry discharges H-1 itself. |
| 2026-08-11 | DESIGN (application — Morgan, prerequisite fold-in, this pass) | **Three more prerequisites folded in per user ruling** (own scope: slices 01 and 05, ADR-0022's status line only — slice-00, slice-04 and ADR-0031 out of lane, owned elsewhere this pass). **(1) The cgroup-OOM diagnosis gap (deferral D-3, reduced form) — the load-bearing item.** `MemoryPlan::derive` (ADR-0082 § D2.3) makes `guest_bytes == cgroup_max_bytes` unrepresentable, but a wrong `reserve_bytes` (still a RED scaffold) still surfaced as an undiagnosable `Failed / WorkloadCrashedImmediately { signal: 9 }` — exactly the class of misclassified ending K3/US-VM-3 exist to refuse, and per SD-4 an *expected* overrun, not a corner case. Closed the **reduced** form only: a new `CgroupAccounting` port (`overdrive-core`, beside `CgroupFs`; `RealCgroupAccounting`/`SimCgroupAccounting`; composed and probed alongside `Vmm` under SD-5's gate) reads the allocation scope's `memory.events` `oom_kill` counter once, synchronously, immediately after the VM exit-watcher's `wait`/`recv()` resolves and before any teardown — in `overdrive-worker`, not via `VmHostState::observe()` (wrong cadence — periodic-tick, node-wide, control-plane-owned — and wrong crate boundary) and not by widening `CgroupFs` (ADR-0083 § A8 already rejected this once, on the trait's own write-only contract, for a related need; reused rather than re-litigated here, since a single already-known cgroupfs path is still a read the trait was never built to expose). The fact threads through the **shared** `ExitEvent` via a new `oom: Option<OomFacts>` field (`ExecDriver` never populates it), and `overdrive-control-plane`'s `worker::exit_observer::handle_exit_event` gains one additive, `DriverType`-agnostic precedence check ahead of its existing `Crashed → WorkloadCrashedImmediately` mapping — **correcting Slice 01's own "exit observer … reused unchanged" Dependencies claim**, which predates this fold-in. Minted a thirteenth `TransitionReason` Cause variant, `VmOutOfMemory { limit_bytes, oom_kill_count }` (ADR-0083 § D5), built in **Slice 01** (not 02–04, unlike the other twelve) alongside the mid-run exit watcher that slice already owns; disposition is ordinary `StoppedBy::Process`, **not** `PlatformReclaimed` — consumes restart budget exactly as any other crash, no exemption (checked against DD-1's ending-class table before assuming). Scope boundary stated explicitly: an OOM during the 30 s boot race (before the beacon wins D3's three-way `select!`) is unaffected and still falls through to the existing boot-failure vocabulary; the live `memory.events` subscription and `ExecDriver`'s own OOM path stay deferred. Landed as ADR-0082 § D8 (new), ADR-0083 § D5 row 13 + a Status/A8 addendum, and slice-01 AC + Behavior + corrected Dependencies text; slice-05 gained a coupling note — the DELIVER pass that measures `reserve_bytes` now has an honest oracle (`oom_kill_count == 0` at residency) instead of an undifferentiated signal read. **(2) `overdrive-init`'s musl targets surfaced in slice-01** (M-5's toolchain half) — `{x86_64,aarch64}-unknown-linux-musl` was already decided in ADR-0082 § D7 but appeared in zero slice files; added as an engineering constraint in slice-01, with the publish/BYO-artifact pipeline correctly left to DEVOPS, cited as GH #264. **(3) ADR-0022's Status line amended** — records supersession-in-part by ADR-0083 (the single `AppState.driver` decision only); ADR-0022's deferral *reasoning* is untouched, closing the index-hygiene gap M-3 named (ADR-0083 already recorded the supersession from its own side). No GitHub issues created; #260–#264 cited by number only, per existing rulings. |
| 2026-08-11 | DESIGN (infrastructure — system designer, prerequisite fold-in) | **Two prerequisites folded in per user ruling that anything this feature needs to work properly lands now, not deferred.** (1) **D-5 undeferred** — `shmem_enabled=advise` was filed as out-of-scope on the reasoning "only bites once volumes land (Slice 04)"; corrected — Slice 04 IS this feature, so a later-slice prerequisite is a feature prerequisite. Slice 04 gains: node bootstrap (`infra/provision/common-system.sh`, NOT the nonexistent Image Factory, NOT the driver) sets the host-wide sysfs knob once, boot-persistent; a WARN-not-refuse disposition (extends the existing `Vmm::probe()` gate — this is a throughput knob, not a correctness/security claim like `[D8d]`'s `--sandbox=namespace`, so SD-5's "substrate lie refuses, capability absence does not" puts it on the non-refusing side) emitting `health.startup.warn` (`reason: "vmm.shmem_enabled_suboptimal"`) once at boot; and a Dependencies-section note that Tier-3 volume-throughput evidence is meaningless against the un-tuned host default. No ADR-0068 amendment — that ADR governs kernel-version pinning policy, not host sysctls, and there is no dedicated provisioning/Image-Factory ADR to amend instead. (2) **ADR-0031:539 corrected** — it named a `[microvm]` TOML table that intake **I-5** deleted in favour of `[vm]` (`DriverType::MicroVm` collapsed into `DriverType::Vm`, one VM-class driver); a one-line supersession note added citing intake Decision I-5, per user instruction to keep the edit minimal (other `[microvm]`-future-table mentions deeper in the ADR, e.g. §§ around lines 803/910-911, are untouched — flagged, not fixed, as out of this pass's scope). |
