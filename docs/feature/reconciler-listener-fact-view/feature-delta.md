...
- **DELIVER regression invariant A (zero-scan):** *"`ServiceMapHydrator::hydrate_desired` issues zero `scan_prefix` calls in steady state."* Assertable via a counting `IntentStore` decorator (sim) that records `scan_prefix` invocations; the test submits S services, runs N convergence ticks, asserts the scan counter is 0 after boot. This mirrors the existing `WriteThroughOrdering` DST-invariant discipline (research recommendation 2).
...
None requiring a GitHub issue at design time. The DELIVER counting-store decorator (invariant A) is in-scope for the fix, not a deferral — and its feasibility is now a **verified fact**: `scan_prefix` is a public method on the `IntentStore` trait (`overdrive-core/src/traits/intent_store.rs:255`), decoratable on `&dyn IntentStore` with no new sim surface. No operator-tunable knobs introduced. No forward pointers.
