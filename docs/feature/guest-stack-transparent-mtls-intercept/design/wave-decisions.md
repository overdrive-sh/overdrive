...
| TRC | Complete all four production Stop emitters. Explicit Operator-stop and absent-intent GC retain R4a's target predicate. Generation replacement fences the current predecessor before filtering/placement: Running or Terminated/None emits Operator Stop, Draining waits, terminal Some(T)+routed emits an exact-T zero-durable tail, and only terminal Some+unrouted may mint/stamp. Service liveness actual facts add exact terminal+route inputs; its existing threshold counter is retained until Terminated exact+unrouted/mismatch, making Terminated/None and exact+routed repairs reachable without a receipt. Existing self-enqueue, AllocStatus watch/Lagged routing, and unconditional relist trigger fresh evaluation. ObservationStore remains the sole durable lifecycle source. |
| NPU | After slot assignment, a workload-netns/VM-TAP provision error runs the existing allocation-keyed teardown before Failed. Host teardown attempts netns → typed-owned host TAP → host veth/route → resolver directory without early exit; absence is benign, diagnostics are bounded by those four stages, and the first typed cleanup error returns through the unchanged port. Release the slot only after all stages succeed; otherwise retain it. Preserve the provision failure as primary in the existing Failed cause and append bounded cleanup detail. No driver/intercept/supervision cleanup applies before fresh `Driver::start`. |
...
| TRC-ARCH-002 | The Stop inventory is exactly explicit Operator, absent-intent SystemGc, desired-generation Operator, and Service liveness. Generation replacement cannot mint/stamp across Running, Draining, Terminated/None, or terminal+routed predecessor state; it preserves a current mismatched target during exact tail repair. Service liveness reuses its existing counter until exact-unrouted/mismatch and hydrates only row terminal plus route membership. Production self-enqueue/watch/Lagged/relist tests must drive both real emitters and prove one repair then no spin. |
| Original-scope network unwind | After assignment, every provision failure invokes the existing four-stage structural teardown and writes Failed with the provision cause primary. All stages run; release follows total success only; cleanup errors retain the slot and are bounded diagnostics. Fresh pre-driver failures own no supervision/interception. Same-id Restart removes old interception/network before replacement work and unwinds every early replacement failure through the same resource-specific path. |
