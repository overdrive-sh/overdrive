...
| TRC | `WorkloadLifecycle` is the production replay owner. Both Operator-stop and absent-intent GC emit Stop for Running, `Terminated/None`, or exact-target `Terminated` with a retained route; exact+unrouted, mismatched terminal, Pending, and Draining emit nothing. Actual hydration receives an infallible process-local route-key snapshot through the exact new `AllocDriverRouteView` / `HydrationContext.alloc_driver_routes` surface and stores the target intersection in `WorkloadLifecycleState.routed_allocations`. Existing self-enqueue, AllocStatus watch/Lagged routing, and unconditional 30-second relist are the triggers. One exact+routed zero-durable tail removes the route; the next fresh diff is empty. ObservationStore remains the sole durable lifecycle source. |
