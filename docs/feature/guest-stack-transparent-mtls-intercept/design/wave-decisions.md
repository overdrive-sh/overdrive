...
| TRC | Complete all four production Stop emitters. Explicit Operator-stop and absent-intent GC retain R4a's target predicate. Generation replacement fences the current predecessor before filtering/placement: Running or Terminated/None emits Operator Stop, Draining waits, terminal Some(T)+routed emits an exact-T zero-durable tail, and only terminal Some+unrouted may mint/stamp. Service liveness actual facts add exact terminal+route inputs; its threshold counter and attempt marker remain replayable while non-Running. On Running, a changed `started_at` resets the old counter before action dispatch, and only a probe completed strictly after that boundary may count. Same-batch Workload restart therefore cannot inherit the historical decision. Existing triggers remain unordered; no receipt/barrier. ObservationStore remains the sole durable lifecycle source. |
| NPU | After slot assignment, a workload-netns/VM-TAP provision error runs the existing allocation-keyed teardown before Failed. Host teardown attempts typed-owned host TAP → host veth/route → owned resolver directory even across sibling failures, then deletes the netns only after all three prove absence. This retains the slot-named netns as the existing boot discovery anchor until dependent residue is gone. Absence is benign, diagnostics stay bounded by four stages, and the first typed cleanup error returns through the unchanged port. Release only after the four-part absence proof. Preserve the provision failure as primary; no new scanner/token/framework. |
...
| TRC-ARCH-003 | Add exactly `#[serde(default)] ServiceLifecycleView.liveness_attempt_started_at: BTreeMap<AllocationId, UnixInstant>`. A Running `started_at` change clears the historical counter and persists the new marker before action dispatch. Hydration accepts only liveness probes completed strictly after that start; equality/absence is stale. Non-Running counter+marker remain for repair; exact-unrouted/mismatch clears both. A frozen real-broker batch must prove Service tail then Workload Restart cannot re-kill the new attempt before a fresh threshold. No ordering/barrier/receipt/cross-read. |
| TRC-ARCH-004 | Structural teardown is host-stranded owned TAP → host veth/route → owned resolver directory, attempting all three siblings, then netns deletion only after their success. The netns name remains the boot slot anchor across every dependent failure/cut; existing boot observe derives the plan and adopts or GCs before allocation. Netns absence permits release only after all dependents were proven absent. Process cuts after every prefix, foreign-TAP refusal, and boot-GC failure/refusal are required. |
