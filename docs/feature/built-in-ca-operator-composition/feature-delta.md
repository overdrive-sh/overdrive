...
| `alloc status` handler | `crates/overdrive-control-plane/src/` (alloc-status read path) | **MODIFY** | Aggregate `obs.issued_certificate_rows()`, project per running alloc the **max-`issuance_ordinal`** row whose `spiffe_id == SpiffeId::for_allocation(workload_id, alloc_id)` (§ D1-AMEND — the strictly-ordered selection key; `issued_at` is retained as an audit fact, NOT the selection key), into `issued_certificates`. |
...
| S-OC-12 | Summary omits cert bytes/key; current = max-`issuance_ordinal` (§ D1-AMEND) | `@integration @real-io @error @driving_port @slice-3 @edd:O05` | ③ | L3 |
