...
| D8 | Hard refusal with explicit `--allow-no-cgroups` dev flag | §4 isolation claim is honest; production safe by default; dev escape hatch absorbs ergonomics objection | ADR-0028 (escape-hatch portion superseded 2026-05-02 by ADR-0034 — flag removed; leak in `StopAllocation` + redundancy with `cargo xtask lima run --`. Hard-refusal half stays.) |
...
| `--allow-no-cgroups` CLI flag | NONE | CREATE NEW | `clap` flag on the existing `serve` subcommand. ADR-0028. **Reversed 2026-05-02 by ADR-0034: this surface is no longer created. Crafter MUST NOT add the flag.** |
...
| `clap` | workspace | MIT/Apache-2 | New `--allow-no-cgroups` flag on `overdrive serve`. **Reversed 2026-05-02 by ADR-0034: flag is not added.** `clap` is still in the workspace dep graph for unrelated subcommands; this row no longer reflects net-new dep usage in this wave. |
...
| ADR-0028 | cgroup pre-flight refusal | Hard refusal at boot on missing cgroup v2 delegation; explicit `--allow-no-cgroups` dev escape hatch with loud startup banner; cgroup v1 hosts get an actionable error. **Escape-hatch portion superseded 2026-05-02 by ADR-0034**: flag removed (structural leak in StopAllocation; redundant with `cargo xtask lima run --`). Hard-refusal pre-flight stays. |
| ADR-0034 | Remove `--allow-no-cgroups` escape hatch (post-ratification supersession of ADR-0028) | `--allow-no-cgroups` deleted from `overdrive serve`; canonical dev path on macOS / non-delegated Linux is `cargo xtask lima run -- overdrive serve`. Hard-refusal pre-flight from ADR-0028 retained unchanged. Single-cut migration per greenfield convention. Ratified 2026-05-02. |
...
| 2026-05-02 | Post-ratification supersession of D8's escape-hatch portion: ADR-0034 deletes `--allow-no-cgroups` from `overdrive serve`. Two reasons: (a) the flag silently leaked workloads — `StopAllocation` constructed `AllocationHandle { pid: None }`, the process-kill path was a no-op for `pid: None`, and the cgroup-kill branch was gated off by the flag, so `stop` returned `Ok(())` while the workload kept running and the next reconciler tick saw `state: Terminated` against a live PID; (b) the canonical dev path is now `cargo xtask lima run --` (per `.claude/rules/testing.md`), which absorbs the dev-ergonomics objection ADR-0028 § Alternative A documented. Hard-refusal pre-flight from D8 / ADR-0028 stays. brief.md §31 rewritten; brief.md ADR index updated (0028 → Superseded in part, 0034 → Accepted); brief.md Linux-only requirements bullet replaces flag reference with Lima wrapper. Single-cut migration per greenfield convention — flag definition + `ServerConfig::allow_no_cgroups` field + `ExecDriver::allow_no_cgroups` field + every gated branch + `--allow-no-cgroups`-gated tests are deleted in one crafter PR. Wave-decisions log is append-only: D8 row and ADR-0028 row retain their original text plus a follow-up note pointing at ADR-0034. — Morgan. |
