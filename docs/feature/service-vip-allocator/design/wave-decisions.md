...
operator submit → parser → spec-digest compute (with `listener.vip = None`) →
admission handler validates `listener.vip.is_none()` for every listener
(AC-06 — resolves Open Q5) →
`ServiceVipAllocator.allocate(spec_digest)` returns `vip` →
`listener.vip = Some(vip)` projection →
IntentStore admission write →
submit-echo renders the assigned VIP (AC-01).
...
| K8 | Admission-level rejection of operator-supplied `vip = Some(...)`, not parser-level | Preserves ADR-0047 `Option<ServiceVip>` field shape; admission is a policy layer | Q5 |
...
| `ListenerRow.vip: Option<ServiceVip>` (ADR-0047 § 4a) | `crates/overdrive-core/src/aggregate/workload_spec.rs` (Slice 06 of `workload-kind-discriminator`) | Field that receives the allocator's output | **REUSE AS-IS** | Forward-compatibility framing from ADR-0047 § 4a preserved verbatim. Field semantics evolve from "operator may pin or platform will allocate" to "always platform-allocated; admission rejects `Some(...)` per AC-06". |
...
**Verdict**: 8 EXTEND, 5 REUSE AS-IS, 4 CREATE NEW (each justified — no existing equivalent). Reviewer gate compliance: every CREATE NEW carries explicit "no existing alternative" justification.
...
VIPs only). DESIGN's resolution of Open Q5 (admission-level
rejection, not parser-level) means:

- **Slice 06 spec shape stays UNCHANGED.** The
  `ListenerRow.vip: Option<ServiceVip>` field shape from ADR-0047
  § 4a is preserved verbatim. The forward-compatibility framing is
  preserved verbatim.
- **The new admission-level check is additive.** A new
  `AdmissionError::VipNotOperatorAssignable { listener_idx }` variant
  + validation in the admission handler. No change to parser code,
  no change to the spec aggregate shape.
- **One `ServiceVip` declaration is deleted** (the duplicate at
  ...
  is a single-cut consolidation; the `Listener` field references the
  surviving canonical newtype.
...
| Q5 | Upstream slice-06 admission policy | Admission-level rejection (not parser-level); preserves ADR-0047 field shape | ADR-0049 § 5 |
...
- KPI-shaping decisions from this DESIGN:
  - K1: counter on admission entry (gated `listener.vip.is_none()`), counter on allocator allocation, counter on admission success
