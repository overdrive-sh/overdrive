...
## Phase 2.16 — Conntrack component diagram (additive L3 update)

**Source:** `docs/feature/phase-2.16-xdp-conntrack/design/architecture.md`
**ADR:** ADR-0044 (per-CPU LRU conntrack map selection)
**Date:** 2026-05-06

C4 Level 1 (System Context) and Level 2 (Container) are unchanged
from Phase 2.2 — no new crates ship in this phase. The L3 component
diagram below is **additive**: every box that existed in the Phase 2.2
L3 continues to exist unchanged; this section adds the two new boxes
(`CONNTRACK_MAP` and `ConntrackMapHandle`) and documents how they wire
into the existing graph.

### C4 Level 3 — Dataplane subsystem (Phase 2.16 additions)

```mermaid
C4Component
  title Component Diagram — Dataplane subsystem (Phase 2.16 — conntrack additions)

  Container_Boundary(ctrl, "overdrive-control-plane (adapter-host)") {
    Component(hydrator, "ServiceMapHydrator", "Reconciler impl (Phase 2.2 — unchanged)", "Unchanged; update_service now increments ServiceGeneration atomically inside EbpfDataplane")
  }

  Container_Boundary(dataplane, "overdrive-dataplane (adapter-host)") {
    Component(ebpf_dp, "EbpfDataplane", "impl Dataplane (EXTEND — Phase 2.16)", "start() now calls probe().await? then install_notrack().await? before admitting traffic; update_service() increments ServiceGeneration atomically with SERVICE_MAP outer-map swap")
    Component(conntrack_handle, "ConntrackMapHandle  [NEW — ADR-0044]", "typed map handle", "BPF_MAP_TYPE_LRU_PERCPU_HASH; key FlowKey (5-tuple, host-order); value FlowEntry; capacity 1_048_576 entries / CPU; per-CPU read via bpf_map_lookup_percpu_elem")
    Component(notrack, "notrack.rs  [NEW]", "Rust module", "install_notrack() / uninstall_notrack() — scoped iptables -t raw NOTRACK rule matching dataplane VIP range only; Drop impl removes rule")
    Component(probe, "probe.rs  [NEW]", "Rust module (ADR-0044 § Decision 5)", "Three Earned Trust probes: LRU_PERCPU_HASH kernel support, iptables -t raw availability, NOTRACK effectiveness; any failure → StartError::ProbeFailed + health.startup.refused")
    Component(loader, "loader (aya::Bpf)", "Rust module (EXTEND)", "Also attaches conntrack_map.o; unchanged XDP + TC attachment")
    Component(swap, "swap.rs", "Rust module (Phase 2.2 — unchanged)", "Atomic HASH_OF_MAPS outer-map fd replacement")
    Component(maglev_userspace, "maglev/{permutation,table}", "Rust modules (Phase 2.2 — unchanged)", "Eisenbud permutation + weighted multiplicity expansion")
    Component(svc_handle, "ServiceMapHandle", "typed map handle (Phase 2.2 — unchanged)", "BPF_MAP_TYPE_HASH_OF_MAPS; outer key (ServiceVip, u16 port)")
    Component(backend_handle, "BackendMapHandle", "typed map handle (Phase 2.2 — unchanged)", "BPF_MAP_TYPE_HASH; key BackendId")
    Component(maglev_handle, "MaglevMapHandle", "typed map handle (Phase 2.2 — unchanged)", "BPF_MAP_TYPE_HASH_OF_MAPS; outer key ServiceId")
    Component(reverse_handle, "ReverseNatMapHandle", "typed map handle (Phase 2.2 — unchanged)", "BPF_MAP_TYPE_HASH; key ReverseKey")
    Component(drop_handle, "DropCounterHandle", "typed map handle (Phase 2.2 — unchanged)", "BPF_MAP_TYPE_PERCPU_ARRAY; index DropClass")
  }

  Container_Boundary(bpf, "overdrive-bpf (binary, target bpfel-unknown-none, #![no_std])") {
    Component(conntrack_map, "CONNTRACK_MAP  [NEW — ADR-0044]", "BPF map declaration", "LruPerCpuHashMap<FlowKey, FlowEntry>; capacity 1_048_576 / CPU; kernel LRU eviction; declared in maps/conntrack_map.rs")
    Component(xdp_prog, "xdp_service_map", "XDP program (EXTEND — Phase 2.16)", "CONNTRACK_MAP lookup runs BEFORE Maglev hash; hit-with-current-gen → fast-path; hit-with-stale-gen → branch on flow_state; miss → Maglev + CONNTRACK_MAP insert (FlowState::SynOnly)")
    Component(tc_prog, "tc_reverse_nat", "TC egress program (EXTEND — Phase 2.16)", "On SYN-ACK: builds forward_key from (reverse_key, OriginalDest); if flow_state==SynOnly promotes to Established in CONNTRACK_MAP")
    Component(sanity, "shared::sanity", "#[inline(always)] helpers (EXTEND — Phase 2.16)", "Adds flow_key_from_packet companion to existing reverse_key_from_packet; same host-order lockstep contract (ADR-0041 § 11 extended)")
  }

  Container(core_id, "overdrive-core::dataplane  [EXTEND]", "NEW newtypes: FlowKey (5-tuple, STRICT discipline), FlowEntry (POD, rkyv), FlowState (#[repr(u8)] enum), ServiceGeneration (newtype)")
  Container(sim, "overdrive-sim::SimDataplane  [EXTEND]", "Per-CPU conntrack mirror: BTreeMap<(cpu_index, FlowKey), FlowEntry> with deterministic LRU eviction; rotate_service_generation / migrate_flow_cpu surfaces; three new DST invariants in invariants/conntrack/")
  System_Ext(kernel, "Linux kernel BPF subsystem", "LRU_PERCPU_HASH eviction policy; bpf_map_lookup_percpu_elem; iptables -t raw NOTRACK hook (netfilter raw table)")

  Rel(ebpf_dp, conntrack_handle, "Owns ConntrackMapHandle; reads per-CPU entries at startup for Earned Trust probe 3")
  Rel(ebpf_dp, notrack, "Calls install_notrack().await? inside start() after probes pass; Drop calls uninstall_notrack()")
  Rel(ebpf_dp, probe, "Calls probe().await? as first action inside start() (compile-enforced ordering)")
  Rel(ebpf_dp, loader, "Loader also brings up CONNTRACK_MAP fd for use by ConntrackMapHandle")
  Rel(ebpf_dp, swap, "Unchanged atomic SERVICE_MAP swap; now also increments ServiceGeneration atomically")
  Rel(conntrack_handle, conntrack_map, "Typed userspace handle over CONNTRACK_MAP fd loaded by aya; PerCpuHashMap<MapData> internal shape")
  Rel(conntrack_handle, kernel, "bpf_map_lookup_percpu_elem / bpf_map_update_elem syscalls")
  Rel(xdp_prog, conntrack_map, "CONNTRACK_MAP::get(&flow_key) before Maglev; CONNTRACK_MAP::insert on miss")
  Rel(xdp_prog, sanity, "Calls flow_key_from_packet (host-order conversion site)")
  Rel(tc_prog, conntrack_map, "CONNTRACK_MAP::get + conditional insert on SynOnly→Established promotion")
  Rel(tc_prog, sanity, "Existing original_dest_to_wire call unchanged")
  Rel(xdp_prog, svc_handle, "Unchanged SERVICE_MAP lookup")
  Rel(xdp_prog, maglev_handle, "Maglev lookup on conntrack miss path only")
  Rel(xdp_prog, backend_handle, "BackendEntry lookup unchanged")
  Rel(xdp_prog, reverse_handle, "REVERSE_NAT_MAP write on forward path (unchanged)")
  Rel(xdp_prog, drop_handle, "DROP_COUNTER increment on drop (unchanged)")
  Rel(tc_prog, reverse_handle, "REVERSE_NAT_MAP lookup on reverse path (unchanged)")
  Rel(hydrator, ebpf_dp, "Calls update_service via Arc<dyn Dataplane> (unchanged action path)")
  Rel(ebpf_dp, core_id, "Constructs FlowKey / FlowEntry / FlowState / ServiceGeneration values")
  Rel(sim, core_id, "SimDataplane mirrors FlowKey / FlowEntry / FlowState / ServiceGeneration shapes")
```

The diagram makes two additions visually explicit against the Phase 2.2 baseline:

1. **`CONNTRACK_MAP` is the fifth BPF map** — `LruPerCpuHashMap<FlowKey,
   FlowEntry>` scoped to dataplane-managed VIPs; kernel LRU eviction is
   the only TTL mechanism in Phase 2.16 (`last_seen_secs` field exists
   for a future per-protocol TTL extension per ADR-0044 § Consequences).
2. **`ConntrackMapHandle` is the fifth typed userspace handle** — mirrors
   `DropCounterHandle`'s per-CPU read pattern; the Earned Trust probe in
   `probe.rs` reads it at startup to verify LRU_PERCPU_HASH is operational
   on the host kernel before any traffic is admitted (CLAUDE.md § Principle
   12 "wire then probe then use").
