...
7. **Kernel-observable alloc-rule accounting** (ADR-0088/0089): the existing
   ...
   TPROXY/mark/accept tail. `overdrive-netlink`'s internal read surface adds
   ...
   `tot_len`/nft-`skb->len` byte total. Shared, inbound, and output-divert rules
   remain unchanged.
