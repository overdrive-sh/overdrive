...
ignored. `intercept-live` means `start_alloc` returned success **and** the
exact outbound rule is observed on that same host-veth.
...
same tuple/original destination must increment that host-veth rule and reach
leg-F, no cleartext copy may reach the external peer path, and the inter-agent
path must carry TLS records. This pins the complete ordering
...
**Reuse tally** (HARD GATE, full table in the feature-delta): 9 REUSE-AS-IS ·
9 EXTEND · 1 CREATE-NEW (the pure `VmTapPlan` value). Zero new crates, ports,
