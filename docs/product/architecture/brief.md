...
| 0044 | XDP per-CPU LRU conntrack table (`BPF_MAP_TYPE_LRU_PERCPU_HASH`) + `FlowKey` / `FlowEntry` / `FlowState` / `ServiceGeneration` newtypes + `iptables -t raw -j NOTRACK` install + three Earned Trust probes (CLAUDE.md § Principle 12) + three DST invariants. Closes the §15 *Zero Downtime Deployments* claim under aggressive canary cutover; closes the kernel-netfilter-conntrack interference symptom in S-2.2-17 (Phase 2.16; pulls forward GH #154) | Proposed |
...
## Phase 2.16 — XDP per-CPU LRU conntrack extension

**Source:** `docs/feature/phase-2.16-xdp-conntrack/design/wave-decisions.md`
**ADR:** ADR-0044 (per-CPU LRU conntrack + NOTRACK + Earned Trust
probes + three DST invariants).
**GH:** #154 (pulled forward from its 2.16 slot to immediately
follow Phase 2.2 Slice 06–08).
**Date:** 2026-05-06.

This section extends Phase 2.2 (§44–§52) with the conntrack-layer
decisions landed by feature `phase-2.16-xdp-conntrack`. The Phase
2.2 stateless-Maglev architecture is preserved verbatim; this
extension adds a 5-tuple → backend table the XDP forward path
writes on first-packet-in-flow and reads on every subsequent
packet, with `iptables -t raw -j NOTRACK` ensuring kernel
netfilter never sees these flows.

### 53. New newtypes — module placement under `overdrive-core::dataplane`

Four additions to `overdrive-core::dataplane/`:

| Type | Module | Purpose |
|---|---|---|
| `FlowKey` | `overdrive-core/src/dataplane/flow_key.rs` (NEW) | 5-tuple `(client_ip, client_port, vip, vip_port, proto)` — pre-NAT key. Host-order in map storage; conversion in `crates/overdrive-bpf/src/shared/sanity.rs::flow_key_from_packet` (companion to existing `reverse_key_from_packet`). |
| `FlowEntry` | `overdrive-core/src/dataplane/flow_entry.rs` (NEW) | POD value `{ backend_id, service_generation, last_seen_secs, flow_state, _pad }`. `last_seen_secs` is a *persisted input* per `development.md` § Persist inputs, not derived state — never a deadline. |
| `FlowState` | `overdrive-core/src/dataplane/flow_state.rs` (NEW) | `#[repr(u8)]` enum, two variants (`SynOnly=0`, `Established=1`). Stable variant set per ADR-0037-style minor-version semantics. |
| `ServiceGeneration` | `overdrive-core/src/dataplane/service_generation.rs` (NEW) | Monotonic counter; incremented by `Dataplane::update_service` on every backend-set rotation; embedded in SERVICE_MAP outer-map slot metadata. |

All four carry the STRICT-newtype discipline (FromStr / Display /
serde / rkyv / proptest) per `development.md` § Newtypes —
STRICT, except `FlowEntry`/`FlowState` which are POD types where
the BPF map value contract is the wire-shape (still serde / rkyv
/ proptest, no FromStr — same shape as the existing `BackendEntry`
in ADR-0040).

### 54. New BPF map — `CONNTRACK_MAP`

| Map | Type | Key | Value | Notes |
|---|---|---|---|---|
| `CONNTRACK_MAP` | `BPF_MAP_TYPE_LRU_PERCPU_HASH` | `FlowKey` | `FlowEntry` | Per-CPU LRU; capacity 1_048_576 entries per CPU. Kernel-side typed wrapper `aya_ebpf::maps::LruPerCpuHashMap<K, V>` (aya-rs research § A.1). Userspace fd extraction via `Map::PerCpuLruHashMap(MapData)` hand-rolled handle. |

The `DropClass` enum (ADR-0040, locked at 6 slots per Q7=B) is
**not** extended in Phase 2.16 — conntrack misses don't drop;
they fall through to Maglev. Conntrack-related drops (the
`flow_state == SynOnly && stale_generation` re-hash path) reuse
the existing `DropClass::NoHealthyBackend` slot when Maglev
returns no live backend after re-hash.

### 55. NOTRACK installation contract

Per ADR-0044 § Decision 4. Production-side installation in
`EbpfDataplane::start`; test-side bridge installation in
`ThreeIfaceTopology::setup` (Slice 06-04, then deleted by Slice
16-03).

The NOTRACK rule is scoped to the dataplane-managed VIP range,
not all traffic. Operator-tunable VIP-range matching is deferred
to a future Phase 3+ slice (same shape as `MaglevTableSize`'s
deferral).

### 56. Earned Trust probes (CLAUDE.md § Principle 12)

`Dataplane::probe()` extended with three substrate-honesty
checks per ADR-0044 § Decision 5:

1. `BPF_MAP_TYPE_LRU_PERCPU_HASH` is creatable on the running
   kernel.
2. `iptables -t raw` is functional on the host.
3. Kernel netfilter conntrack honours NOTRACK (probe writes a
   bogus NOTRACK rule, sends a synthetic packet, confirms no
   conntrack entry was created, removes the rule).

`SimDataplane::probe()` returns `Ok(())` unconditionally — the
simulation does not have a kernel to lie about. Per
`development.md` § "Production code is not shaped by simulation"
the production probe surface is shaped by the production
substrate's actual failure modes; the simulation mirrors the
contract, not the production-shape.

### 57. Three new DST invariants
...
| `ConntrackEventuallyConvergesAfterRotation` | Eventually: stale-generation entries age out within the LRU expiry window. |
| `NoTrackProbeRefusesStartOnFailure` | Always: if any of the three Earned Trust probes fails, `EbpfDataplane::start` returns Err with `health.startup.refused`. |

All three live in
`crates/overdrive-sim/src/invariants/conntrack/` and run on
every PR per `.claude/rules/testing.md` § Tier 1.

### 58. Quality-attribute scenarios — Phase 2.16 (extending §50)
...
| ASR-2.16-04 | Operability — startup refusal on substrate-lie | Test environment with `BPF_MAP_TYPE_LRU_PERCPU_HASH` disabled / `iptables -t raw` missing / NOTRACK ignored. | `EbpfDataplane::start` returns Err with `health.startup.refused`; no partial-state dataplane. (DST: `NoTrackProbeRefusesStartOnFailure`.) |

### 59. Updated handoff annotations — Phase 2.16 (extending §52)

To DEVOPS — required CI checks gain:

- `cargo xtask verifier-regress` baseline updates expected (the
  conntrack lookup adds two map operations to the forward hot
  path; perf-gate gates the delta).
- `cargo xtask xdp-perf` baseline updates expected (forward path
  pps may regress within the 5 % gate; if not, raise an ADR
  amendment per ADR-0044 § Alt 4 rejection rationale).
- No new `cargo nextest` invariants beyond the three already
  named in §57.

External integrations in Phase 2.16: **none**. Same posture as
Phase 2.2 — kernel-bound, not external. NOTRACK installation
invokes the system `iptables` binary (treated as substrate, not
external integration; covered by Earned Trust probe §56).
