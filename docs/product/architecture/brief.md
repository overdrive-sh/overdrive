...
| 6 | any shim arm that concludes a terminal-row authorship attempt | `*` → ∅ | accepted write: authored. `StopAllocation` read/write error, two-LWW-loser exhaustion, or cancellation after cleanup: abandoned. An exact target terminal replay is accepted-tail repair. The Stop route is removed only for accepted/exact-terminal outcomes and retained for error/exhaustion/cancellation — ADR-0083 amendment 2026-08-31 |
...
/// store closure is still resolving commit/rollback; a later current read
...
`Ok(())`, and yields to level-triggered convergence. Store `Err` returns the
...
subscription send attempt; exact-terminal replay then removes the retained
route without repeating cleanup or notifications. Once `Ok(Some)` reaches the
shim, release, route removal, and direct best-effort event send are a contiguous
