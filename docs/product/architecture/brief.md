...
### Public-ingress gateway — System topology (ADR-0104/0117–0121, Accepted 2026-09-13)
...
Application ADR-0106–0111/0122–0132 handoff references are in
...
System ADR-0104/0117–0121 own the high-level placement, credential-consumption,
packet-entry, consistency, trust and listener-gate choices. Application
ADR-0106–0111/0122–0132 own their independently reversible realization decisions; exact
...
### Public-ingress gateway Route/domain model (Route ADR-0105; Domain ADR-0112–0116/0133, Accepted 2026-09-13)
...
System ADR-0104/0117–0121 resolve the former system gap with demand-gated ADR-0053 TEACH,
...
System ADR-0104/0117–0121 resolve the former `DESIGN-GAP-PIG-1`. Active flow is Route → exact
...
[ADR-0114](adr-0114-dataplane-selection-receipt-ownership.md),
[ADR-0115](adr-0115-derived-gateway-frontend-demand-lifecycle.md),
[ADR-0116](adr-0116-state-based-public-ingress-domain-ownership.md) and
...
| 0104 | **Embed first public ingress in `overdrive serve`** — one selected node/process/ServerHandle owns the gateway and public-listener lifecycle; not an Allocation, daemon or external proxy | Accepted |
| 0105 | **Singleton Public Route Set aggregate** — one optional Route entity atomically owns first-slice cardinality/conflict; stable `(WorkloadId, port, tcp)` target; public exposure remains outside Service intent | Accepted |
| 0106 | **One coherent Gateway Application admission owner** — the active owner alone stages demand, publishes whole Route/frontend/certified-key generations, drains retained users and owns public-listener admission; singleton Route cardinality remains ADR-0105. | Accepted |
| 0107 | **Producer-neutral Public Certified-Key Custody** — one protected current credential boundary shared by manual input and the later GH #57 Workflow producer. | Accepted |
| 0108 | **Existing cgroup-BPF Dataplane selects gateway backends** — the gateway supplies one exact Service Frontend; BPF alone performs Maglev selection and destination rewrite. | Accepted |
| 0109 | **Dedicated Gateway SVID identity lifecycle** — one non-allocation slot owns issue/audit/renew/restart/drain-ordered drop without fabricating Allocation identity. | Accepted |
| 0110 | **Public listener requires TLS 1.3** — rustls accepts only TLS 1.3 with the custody-owned Web-PKI identity. | Accepted |
| 0111 | **Separate redacted operator gateway status** — ID-scoped application/custody plus independent hydration projection over operator mTLS; no public response-state or secret/receipt detail. | Accepted |
| 0112 | **Distinct Public Certified-Key Custody domain boundary** — Public Certified Key aggregate remains separate from Route, operator HTTPS, internal SPIFFE and allocation IdentityMgr; lifecycle mechanics are owned elsewhere | Accepted |
| 0113 | **Dedicated Gateway Identity Slot/lifecycle** — non-allocation subject, issue/audit/renew/restart/drop and sole gateway-client consumer; allocation identity unchanged | Accepted |
| 0114 | **Service Dataplane selection-receipt ownership** — transient cookie+ServiceKey cgroup-BPF receipt remains with the selector; gateway observes but never owns/selects | Accepted |
| 0115 | **Derived Gateway Frontend Demand lifecycle** — exact staged/current/draining frontend union, staged-before-swap and drain-before-withdraw; no persisted demand truth | Accepted |
| 0116 | **State-based public-ingress ownership** — aggregates, reconcilers and derived views reuse Intent/Observation/View boundaries; no ES/CQRS/event log/new store | Accepted |
| 0117 | **Separate public-certificate acquisition from runtime consumption** — manual input first and ACME #57 later publish through one producer-neutral Public Certified Key boundary; listener/Route forwarding stays acquisition-independent | Accepted |
| 0118 | **Use existing cgroup-BPF Service dataplane for gateway upstream** — exact Service Frontend enters ancestor connect4/Maglev selection; demanded Path-A is TEACHed; XDP wire path unchanged and no userspace LB | Accepted |
| 0119 | **Keep Gateway Application and Service Dataplane generations independent** — no false unified transaction; each connect correlates actual BPF selection through its transient receipt | Accepted |
| 0120 | **Separate public TLS from gateway-SVID workload mTLS** — Web-PKI terminates at the gateway, then a dedicated gateway identity pins the exact BPF-selected workload peer; three credential domains remain distinct | Accepted |
| 0121 | **Gate public listener without redefining workload/Service readiness** — TCP/443 admission depends only on gateway-owned prerequisites and never changes Allocation Running, Service Stable, Backend Eligibility, BPF Hydrated or operator HTTPS meaning | Accepted |
| 0122 | **Preserve the last usable Public Certified Key during replacement** — prove/build/seal/persist before Current publication and write status last; pre-publication failure retains the prior generation. | Accepted |
| 0123 | **Demand-gate Path-A Service-map teaching** — ServiceMapHydrator teaches only exact Staged/Current/Draining Gateway Frontend Demand and acknowledges the applied revision before promotion. | Accepted |
| 0124 | **Gateway-connect selected-backend receipt** — correlate one registered `(SO_COOKIE, ServiceKey)` connect with transient `Selected(BackendId) \| NoBackend` and clean it on every return. | Accepted |
| 0125 | **Commit-gated backend-identity publication** — reserve identity, commit the outer Service-map swap, then expose immutable BackendId-to-applied-workload identity under one Dataplane guard. | Accepted |
| 0126 | **Exact-peer gateway-client mTLS** — accept only the workload SPIFFE URI SAN resolved from the receipt-selected BackendId. | Accepted |
| 0127 | **Public HTTP/1.1 only** — Hyper serves one explicit HTTP version; HTTP/2 and HTTP/3 remain disabled. | Accepted |
| 0128 | **Canonical-authority plus raw-path Route selection** — exact SNI/Host equality and raw Exact/segment-prefix path determine the singleton Public Route. | Accepted |
| 0129 | **Explicit public proxy-header policy** — remove hop-by-hop/untrusted forwarding fields and author canonical Forwarded/Via provenance. | Accepted |
| 0130 | **Fixed finite public runtime limits** — one private first-slice policy bounds admission, parsing, bodies, cleanup and deadlines without operator knobs. | Accepted |
| 0131 | **Stream public proxy bodies with backpressure** — preserve body octets without whole-body memory or disk buffering. | Accepted |
| 0132 | **One upstream attempt per public request** — no retry, replay, hedge or alternate-backend attempt. | Accepted |
| 0133 | **Service Dataplane Applied Backend Identity Association** — receipt BackendId resolves only through the Dataplane-owned readable Applied association; gateway/address indexes cannot publish identity | Accepted |
...
### Public-ingress gateway application architecture (ADR-0106–0111/0122–0132, Accepted 2026-09-13)
...
| ADR | One decision |
|---|---|
| [0106](adr-0106-public-ingress-route-and-gateway-application-owner.md) | One active owner for coherent Gateway Application admission/listener lifecycle |
| [0107](adr-0107-producer-neutral-public-certified-key-custody.md) | Producer-neutral protected Public Certified-Key Custody |
| [0108](adr-0108-existing-cgroup-bpf-gateway-backend-selection.md) | Existing cgroup-BPF Dataplane remains the only gateway backend selector |
| [0109](adr-0109-dedicated-gateway-svid-identity-lifecycle.md) | Dedicated non-allocation Gateway SVID identity lifecycle |
| [0110](adr-0110-public-listener-tls13-only.md) | TLS 1.3 only on the public listener |
| [0111](adr-0111-redacted-operator-gateway-status.md) | Separate redacted operator gateway-status projection |
| [0122](adr-0122-preserve-last-usable-public-certified-key-replacement.md) | Preserve-old Public Certified-Key replacement transaction |
| [0123](adr-0123-demand-gated-path-a-service-map-teach.md) | Demand-gated Path-A Service-map teaching |
| [0124](adr-0124-gateway-connect-selected-backend-receipt.md) | Per-connect BPF-selected BackendId receipt |
| [0125](adr-0125-commit-gated-backend-identity-publication.md) | Commit-gated applied backend-identity publication |
| [0126](adr-0126-exact-peer-gateway-client-mtls.md) | Receipt-selected exact-peer gateway-client mTLS |
| [0127](adr-0127-public-http11-only.md) | Public HTTP/1.1 only |
| [0128](adr-0128-canonical-public-route-match-key.md) | One canonical authority/raw-path Public Route match key |
| [0129](adr-0129-public-proxy-header-policy.md) | Explicit public proxy-header policy |
| [0130](adr-0130-fixed-finite-public-runtime-limits.md) | Fixed finite first-slice public runtime limit policy |
| [0131](adr-0131-stream-public-proxy-bodies-with-backpressure.md) | Streaming request/response bodies with backpressure |
| [0132](adr-0132-single-upstream-attempt-per-public-request.md) | One upstream attempt per public request |
