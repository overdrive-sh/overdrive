...
**Proposed 2026-09-11 refinement — pending independent architecture review.**
ADR-0089's later selected-TAP attachment exposed one additional CH read: while
constructing virtio-net under Landlock, CH reads the selected allocation TAP's
flags through `/sys/class/net/<tap>`. The bounded production launch fails with
`Permission denied` and never reaches `Running` when only this leaf read grant
is removed. A networked VM therefore has exactly two explicit rules in order:
the selected TAP sysfs leaf with `access=r`, then this exclusive run directory
with `access=rw`. The TAP grant neither weakens nor replaces directory
exclusivity; `/sys/class/net`, sibling TAPs, alternate paths and TAP write
access remain forbidden. Full private/public composition shape: ADR-0082's
proposed 2026-09-11 amendment; attach cross-reference: ADR-0089's proposed
...
flowchart TB
    ...
    subgraph HOSTSTATE["Per-allocation host state (SD-2)"]
        direction TB
        RUN["/run/overdrive/vm/&lt;alloc&gt;/ — <b>tmpfs</b><br/>vsock UDS · beacon UDS · CH API socket<br/><i>exclusive: run-dir Landlock grant is rw</i><br/><i>networked VM proposal: selected TAP sysfs leaf is separate r grant</i><br/>survives serve restart · clears on host reboot"]
...
Accurate status: `vm-reclamation` has landed on `main` as the 8th reconciler and
is now the **first live** host-backed, resync-only consumer of this partition — it
declares a `resync_schedule` (30 s / `LocalNode`, ADR-0084 §4), driven by the
generic Piece A cadence phase that replaced its former hardcoded loop sweep, and
hydrates `actual` live from the host. The other seven reconcilers are row-backed.
The Bar-2 veth/XDP promotions (#197/#199) remain the next host-state candidates.
...
(exact Rust in ADR-0084). RN-3 = **MOOT** (no cache ⇒ nothing to bound). What ships:
...
hydration ownership is untouched. See ADR-0084 and the Application Architecture §
...
### Public-ingress gateway — System topology (ADR-0114/0127–0131, Accepted 2026-09-13)

**Authority boundary.** Current intent is GitHub issue #54 (node-agent-
embedded `hyper` + `rustls`, in-process BPF map access) plus explicit user
corrections. Current code defines reusable facts. The whitepaper is contextual
and non-authoritative. DISCUSS/SPIKE inputs remain absent; no story, KPI or
measured capacity is inferred.
...
(certificate acquisition/consumption),
...
(trust transition), and
[ADR-0131](adr-0131-gate-public-listener-without-redefining-workload-service-readiness.md)
(listener gates).

**System decision.** `overdrive serve` owns a working gateway.
`ManualCertifiedKeySource` supplies `PublicCertifiedKeyCustody`, which validates
and builds usable rustls material, seals via `PublicCertifiedKeyAeadCodec`,
persists the complete `PublicCertifiedKeyV1`, atomically publishes the opaque
Current/Usable snapshot, and only then writes `PublicCertifiedKeyStatusRowV1`.
`GatewayApplicationOwner` publishes one atomic Route/frontend/resolver admission
object, writes `GatewayApplicationStatusRowV1`, and derives
staged+current+draining exact Service-frontend demand. `GatewayControl` serves
the active rows plus relevant hydration through operator-mTLS
`GET /v1/gateway/status`. ServiceMapHydrator
TEACHes Path-A only for that demand into the existing
`SERVICE_MAP` Maglev inner table and `BACKEND_MAP`; undemanded Path-A remains
gated. The gateway connector creates
a socket inside the existing cgroup-attach scope, obtains `SO_COOKIE`, and asks
Dataplane to register only `(cookie, exact ServiceKey)` before `connect(2)`.
`cgroup_connect4_service` performs BPF-owned Maglev selection/destination
rewrite and records the actual BackendId; Dataplane resolves that id to the
exact applied `Backend.alloc` for gateway-SVID expected-peer mTLS.
`HostGatewayClientMtls` stays under the existing mTLS owner, reuses the current rustls
chain/single-SPIFFE-SAN verifier, and adds the equality current authn-only
production does not yet exercise; it does not fabricate `AllocationId` or use
the transparent kTLS pump. After initial
prerequisites pass, `serve` binds IPv4 TCP/443. ACME remains #57.

**Traffic/trust path.** Public TLS/Host/path → exact Service Frontend →
registered socket connect through the already-attached cgroup BPF hook →
selected-BackendId receipt → exact applied workload identity → gateway-SVID
mTLS over the existing Path-A veth → workload-netns inbound nft-TPROXY/
agent-light terminator → plaintext workload → same authenticated-socket response.
The gateway never reads backend rows or maps and never selects in userspace.

The existing XDP wire path remains unchanged. The cgroup hook is selected for
this host-originated connect because it already covers `serve` and ADR-0053
proved it fires for Path-A; a gateway netns/veth created solely to enter XDP
would add a larger packet-entry and reverse-path lifecycle.

`GatewayIdentitySlot` plus `GatewaySvidLifecycle` own non-allocation issue/hold/
reissue/restart/drain-ordered drop using existing CA/audit primitives. Only
`HostGatewayClientMtls` may use private material; the HTTP runtime receives an
authenticated stream, not raw private material. `AT-PIG-E2E-1` proves the full
production composition.

**Lifecycle.** Gateway Applied and BPF applied generations remain independent;
the receipt pins the actual BackendId, whose process-lifetime id resolves to the
exact applied `Backend.alloc`. Dataplane installs that immutable association
as non-readable `Reserved` under its serialized commit guard before BPF work,
performs the fallible map writes, commits selection with the outer Service-map
swap, then infallibly publishes it `Applied` before
releasing the guard. A receipt identity read blocks on the same guard and can
observe only Applied. Associations remain immutable until restart; withdrawal
removes the Service-map slot first, so old receipts cannot be rebound. Boot
applies BPF+identity and may first prove the registered-connect/hook/receipt
path with the sentinel ServiceKey's canonical `NoBackend`, without gateway
identity or mTLS. After Gateway Identity is Current, the actual Service
Frontend probe treats `NoBackend` as bind-valid 503 capability; only its
`Selected` branch performs exact-peer mTLS. Gateway Application is promoted and
TCP/443 binds only after that branch-specific capability result succeeds.
Shutdown drains and clears transient intents/receipts. Public 404/502/503 and
operator status are separate.

The full option matrix, source-of-truth table, component/reuse analysis, five
Lifecycle Gate Ownership declarations, unaffected-state scenarios and exact
System ADR-0114/0127–0131, Route ADR-0115, Domain ADR-0122–0126/0143 and
Application ADR-0116–0121/0132–0142 handoff references are in
[the feature delta](../../feature/public-ingress-gateway/feature-delta.md);
its [Application contract sections](../../feature/public-ingress-gateway/feature-delta.md#wave-design--ref-application-decisions)
are the sole exact API/schema contract.

#### Canonical C4

System Context, Container and Component views are maintained once at the
[canonical public-ingress C4](c4-diagrams.md#public-ingress-gateway-canonical-c4).
System ADR-0114/0127–0131 own the high-level placement, credential-consumption,
packet-entry, consistency, trust and listener-gate choices. Application
ADR-0116–0121/0132–0142 own their independently reversible realization decisions; exact
API/schema contracts live only in the feature delta's Application `[REF]`
sections.
...
<a id="public-ingress-gateway-route-domain-model"></a>

### Public-ingress gateway Route/domain model (Route ADR-0115; Domain ADR-0122–0126/0143, Accepted 2026-09-13)

**Authority and verdict.** GitHub #54 plus the user's rulings govern; the
whitepaper is context only. Public Ingress and Public Certified-Key Custody are
new contexts. Service Dataplane and Workload Identity are extended with the
responsibilities their existing selection/trust boundaries own. The state-based
aggregates are Public Route Set, Public Certified Key and the in-process
single-slot Gateway Identity Slot. ACME/TCP/80 remain outside the slice under
GH #57.

System ADR-0114/0127–0131 resolve the former system gap with demand-gated ADR-0053 TEACH,
registered `(SO_COOKIE, ServiceKey)` cgroup-BPF selection and receipt,
Dataplane-owned `BackendId → exact applied Backend.alloc` identity, and a
dedicated non-allocation gateway SVID holder plus exact-peer gateway-client
rustls. `serve` binds IPv4 TCP/443 only after its gates. The existing XDP wire
path remains unchanged and the gateway never selects a backend in userspace.

#### Context map

Public Ingress is new because no current context owns public host/path exposure.
Certified-Key Custody is distinct from both private CAs. Service Dataplane stays
separate because desired Route Target and actual cgroup-BPF selection are
different concepts/owners. Workload Identity owns gateway credential custody
and peer authentication, never BPF choice. The arrows carry exact values, not
backend enumeration or API signatures.

The detailed [Route/domain context map](c4-diagrams.md#public-ingress-gateway-route-domain-context)
is maintained only in `c4-diagrams.md`.

#### DD-PIG-1 — Public Route Set owns the first-slice Route invariant

The Public Route Set has one fixed aggregate identity and is empty or contains
one Route entity. `RouteId` is the entity identity, not an independent aggregate
key. Its universe is
`{route: None | Some({RouteId, RouteGeneration, PublicHostname, PathMatch,
ServiceListenerReference, PublicCertifiedKeyId})}` plus ordered command facts.
One key makes competing Route claims atomic.

Route Target is stable `(WorkloadId, port, tcp)`. Resolution proves current
Service intent declares that listener and has a platform-issued VIP, yielding
the exact existing Service Frontend `(ServiceVip, port, tcp)`. Route never
persists derived `ServiceId`/VIP, backend, Allocation ID or SPIFFE ID.
Certified-key reference resolution proves the key aggregate exists and its
hostname corresponds; usability remains separate.

Hostname is canonical lowercase ASCII DNS without wildcard/IP/terminal dot.
Path Match is raw-path `Exact` or segment prefix before percent decoding;
`/api` does not match `/apix`. Exact outranks same-path prefix, then longer
prefix wins. Multiple candidates are not authorized now.

| Command | Declared delta | Complement equality |
|---|---|---|
| Declare | empty → full Route; Declared then Accepted | All Service/key/identity/dataplane state |
| Replace same ID | route fields + generation atomically; Replaced then Accepted | Aggregate/Route IDs and all external state |
| Withdraw same ID | Route → empty; Withdrawn | Service/key/identity/BPF unchanged until Gateway Application performs ordered admission/demand drain |
| Competing ID / invalid / exact replay | Empty; cause-distinct conflict/rejection or no-op | Whole universe and all external state |

Vernon's rules hold: one small root owns the real slot invariant; external
references resolve outside it.

#### DD-PIG-2 — Public Certified Key is producer-neutral protected state

`PublicCertifiedKeyId` is the only Route credential reference. Authorized state
is absent or `{id, hostname, current CertifiedKeyGeneration}`, where the
generation contains the protected Origin Private Key, Public Certificate chain,
validity inputs and non-secret provenance.

The chain must cover hostname, match the key and satisfy the approved profile.
Install/replace swaps the complete pair atomically under an expected-current-
generation guard. `usable now` is derived from persisted validity plus live
clock; key bytes/manual paths/raw PEM/`valid`/compiled rustls tables never enter
Route, status, handlers or aggregate state beyond protected key material.

| Command | Declared delta | Complement equality |
|---|---|---|
| Install | absent → complete generation; CertifiedKeyInstalled | Route, Service/dataplane, internal identity |
| Replace | whole generation atomically; CertifiedKeyReplaced | Public Certified Key ID/hostname and all external state |
| Exact replay | Empty; no-op | Whole universe and all external state |
| Missing/invalid/unusable/stale refresh | Empty custody delta; install-failure status only | Preserve current generation while usable; all external state unchanged |
| Current generation becomes Unusable/expired | Retain protected record; derive/publish Unusable | Route and non-custody state; listener/application follows its own gate |

Manual acquisition ends at this command. GH #57 later may call the same boundary
after its own finite Workflow to install/replace only; it cannot add implied
withdrawal or change custody/runtime contracts.

#### DD-PIG-3 — Frontend demand and cgroup-BPF selection stay Dataplane-owned

Public Ingress resolves only the exact Service Frontend and publishes derived
Staged/Current/Draining Gateway Frontend Demand. ServiceMapHydrator TEACHes
Path-A into the existing shared Service maps only for that demand. The gateway
never reads/Watches `ServiceBackendRow`, filters health, holds candidates or a
round-robin cursor, chooses `Backend.alloc`, or touches raw maps.

Before a host-originated upstream connect, the connector registers exactly
`(SO_COOKIE, ServiceKey)`. The already ancestor-attached
`cgroup_connect4_service` selects through the existing `SERVICE_MAP` Maglev
inner table and `BACKEND_MAP`, rewrites the destination and emits a BPF
Selection Receipt `Selected(BackendId) | NoBackend`. This is cgroup-BPF Service
Selection, not XDP. Existing XDP wire forwarding remains unchanged.

Dataplane first reserves the process-lifetime
`BackendId → exact Backend.alloc (SpiffeId)` association as non-readable,
performs the fallible BPF work, commits through the guarded outer Service-map
swap, and only then promotes the association to Applied/readable. Receipt
identity resolution reads Applied only; readers never observe Reserved.
Address/BackendIndex reread and gateway preselection are not authoritative for
the connected socket.

#### DD-PIG-4 — Active Gateway Application and dedicated Gateway Identity Slot

`GatewayApplicationOwner` atomically publishes one immutable Route Generation +
exact Service Frontend + opaque Certified Key resolver snapshot for new
connections. Each connection retains that generation through public TLS/HTTP.
Staged demand precedes the swap; superseded Current becomes Draining and keeps
its frontend demanded until its last connection closes. Gateway Applied and BPF
Hydrated remain independent; the application contains no backend/BPF/identity
state.

Workload Identity owns one in-process Gateway Identity Slot for
`spiffe://overdrive.local/gateway/<node-id>`. Its dedicated lifecycle reuses
`issue_and_audit`, issues on empty, reissues near expiry and restart, and drops
after disable/shutdown stops admission and drains accepted work. Audit precedes hold; failed reissue
preserves the old generation only while usable. Private material is never
persisted and only gateway-client mTLS may consume an opaque use capability.
Allocation `IdentityMgr`/`IdentityRead` remains unchanged.

Gateway-client rustls presents that SVID and requires the workload SPIFFE SAN
to equal the receipt-selected Backend Identity before body delivery. Precise
status terms include Gateway Identity Current, Gateway Frontend Demand Applied,
Gateway Applied Generation, BPF Hydrated, Public Listener Bound and Selected
Peer Authenticated. Operator-mTLS status remains distinct from public
404/502/503; no generic Ready/Healthy/Programmed synonym is introduced.

#### DD-PIG-5 — Lifecycle, occurrence and reuse

- Disabled activates no Route owner/key consumer/frontend demand/gateway
  identity/connect registry/public listener; existing workload/Service/cgroup/
  XDP behavior is unchanged.
- Cold boot loads Route/key, resolves frontend, issues/audits/holds gateway SVID,
  stages and applies demand, probes receipt+identity, applies Gateway
  Application, then binds TCP/443. No partial listener precedes the gates.
- A new generation publishes Staged demand before admission swap; superseded
  Current becomes Draining; demand survives until its last connection closes.
- Route deletion withdraws Route intent/admission/demand only; protected Public
  Certified Key custody is unchanged.
- Missing/invalid/unusable refresh preserves the usable current key. When it
  expires, custody retains the protected record, publishes Unusable and the
  application/listener follows the active fail-closed gate until replacement.
- Gateway identity disable/expiry stops new admission/upstream handshakes,
  drains authorized users and only then drops/replaces gateway SVID material.
- NoBackend is a valid receipt and public 503; missing/mismatched receipt,
  wrong-valid-peer, disconnect or upstream failure are request-scoped 502 with no
  backend retry/plaintext fallback.
- Restart destroys sockets/receipts before rebuilding BackendId associations and
  gateway SVID, reruns gates and binds only after success. Shutdown reverses
  admission/drain/receipt/demand/identity ownership in that order.

Gateway issuance reuses the existing audit occurrence; held/current remains the
in-memory slot. Route/key/identity/application are state plus synchronous facts;
connect receipts/request outcomes are transient. Operator status is separate
from public 404/502/503. No ES/CQRS/workflow/new store is added.

| Existing surface | Decision |
|---|---|
| Service Listener/VIP/ServiceId/ListenerFacts | Reuse inputs; extend stable target→exact frontend projection |
| ServiceLifecycle/ServiceBackendRow | Indirect authority only; no gateway read |
| ServiceMapHydrator | Extend accepted owner with demand-gated ADR-0053 Path-A TEACH |
| SERVICE_MAP/BACKEND_MAP + cgroup_connect4_service | Reuse maps/ancestor attach; extend registered cookie+ServiceKey arm and receipt; XDP wire path unchanged |
| BackendIdAllocator + applied identity | Reuse monotonic no-reuse; create Dataplane-owned immutable association with Reserved/non-readable → guarded outer-map commit → Applied/readable ordering |
| BackendIndex/ResolvedBackend | Reuse identity semantics only; never reread address for receipt resolution |
| IntentStore/envelope discipline | Extend with singleton Public Route Set family |
| KEK/typed-codec + rustls resolver experience | Extend pattern; create Public Certified Key aggregate |
| ca_issuance::issue_and_audit + issuance row | Reuse as-is for exact gateway subject |
| Workload SvidLifecycle + IdentityMgr/IdentityRead | Reuse policy/holder patterns; create dedicated gateway lifecycle and single-slot holder; never extend allocation keying |
| mTLS verifier + transparent pump | Reuse chain/SPIFFE verifier and extend exact equality; create gateway-client entry under same owner; do not reuse allocation/kTLS pump entry |
| Workflow engine/instant-acme | No change; outside feature under GH #57 |
| GatewayApplicationOwner + frontend demand | Create derived no-store capability with staged-before-swap/current/draining-after-swap lifecycle |

#### DD-PIG-6 — Active flow and `AT-PIG-E2E-1`

System ADR-0114/0127–0131 resolve the former `DESIGN-GAP-PIG-1`. Active flow is Route → exact
Service Frontend → staged/current/draining demand → demand-gated Path-A TEACH →
`(SO_COOKIE, ServiceKey)` registration → ancestor cgroup-BPF Maglev selection/
rewrite → `Selected(BackendId) | NoBackend` receipt → Dataplane immutable
BackendId→applied-`Backend.alloc` join → gateway-SVID exact-peer rustls →
workload response. This host-connect selection is not the unchanged XDP wire
path, and the gateway never enumerates/selects backends.

`AT-PIG-E2E-1` is the real built-default-binary Tier-3 walking skeleton, never a
spike. It proves real `serve`, Service/Route deploy, manual test-CA custody,
external TLS, registered cgroup-BPF selection, actual receipt identity,
wrong-valid-peer rejection, NoBackend→503, receipt failure→502, response,
withdrawal/drain/restart cleanup and disabled-feature nonmutation. Production
EDD separately proves a publicly trusted chain.

#### DD-PIG-7 — Domain alternatives

| Choice | Selected | Rejected viable models |
|---|---|---|
| Route ownership | Singleton Public Route Set | Per-Route + global claim adds cross-aggregate coordination; Service-embedded exposure conflates public and workload lifecycle |
| Public Certified Key | Distinct custody context | Route-owned key couples rotation to routing; allocation `IdentityMgr` conflates protected Web-PKI with ephemeral SPIFFE |
| Gateway SVID | Dedicated non-allocation slot/lifecycle | Synthetic AllocationId falsifies ownership; generalized holder key union broadens every allocation identity path |
| History/read model | State aggregates + reconcilers + derived views | ES adds secret/transient replay and retention; CQRS adds projection lag/source duplication without a query/scale driver |

All reuse existing storage/identity/dataplane patterns at their current trust
boundaries. Detailed drivers and trade-offs are recorded in the decision-
specific ADRs below; the manual-cert/BPF-owned-selection vertical slice is
unchanged.

The Tier-1 glossary, aggregate universes/deltas, event model, reuse table and
handoff are in `docs/feature/public-ingress-gateway/feature-delta.md`; the
[Route/domain context map](c4-diagrams.md#public-ingress-gateway-route-domain-context)
lives only in the C4 artifact. Decisions are
...
[ADR-0124](adr-0124-dataplane-selection-receipt-ownership.md),
[ADR-0125](adr-0125-derived-gateway-frontend-demand-lifecycle.md),
[ADR-0126](adr-0126-state-based-public-ingress-domain-ownership.md) and
[ADR-0143](adr-0143-dataplane-applied-backend-identity-association.md).
...
| 0069 | **Transparent mTLS via a universal agent-light L4 proxy** — folds #222 into #26; ONE enforcement mechanism for ALL workload kinds (process/exec, WASM, microVM, unikernel). Workload's outbound TCP transparently intercepted (`cgroup_connect4`-rewrite default / TPROXY alt) to an agent-owned plaintext leg F; the agent drains plaintext losslessly + rustls TLS 1.3 handshake on a peer-facing leg B presenting the held SVID (read via `IdentityRead`, never minted/cached) + arms kTLS on leg B (reusing `ktls::KtlsStream` for control records). Steady state (all four pumps agent-light zero-copy `splice`; NO sockmap/psock on any leg): **forward F→B = BLOCKING `splice` into leg B's kTLS-TX (`tls_sw_sendmsg`/`MSG_SPLICE_PAGES` encrypts in-kernel)**; **return B→F = non-blocking `splice(2)` via `tls_sw_splice_read` on a plain kTLS-RX leg (~1 splice/record)**; the in-memory pre-arm `prelude` keeps a `write_all`. *(Forward-mechanism history: an agent-idle sockmap EGRESS-redirect (`bpf_sk_redirect_map`, flags=0) RETIRED 2026-06-13 — its `MSG_DONTWAIT`-backlog stalled ~10–15% of records — for a `read → write_all` copy; that copy RETIRED 2026-08-26 for the blocking splice, proven lossless 15/15 under send-buffer exhaustion (increment-M). `splice` into kTLS-TX does NOT lose records — the loss class was non-blocking `MSG_DONTWAIT` delivery, not `splice(2)`. ADR-0069 amended 2026-06-13 + 2026-08-26.)* New driven port `MtlsEnforcement` in `overdrive-core` (does NOT fit `Dataplane` — per-connection socket ops, not map writes) + host adapter (`adapter-host`, over kTLS/splice/cgroup_connect4/nft-TPROXY, consumes `IdentityRead`; the sockops/sk_msg/sockmap surface retired with the forward redirect) + `SimMtlsEnforcement` for DST. Earned-Trust `probe()` (wire→probe→use; sentinel handshake + kTLS-arm + forward-encrypt blocking-splice round-trip + splice-read, refuse-to-start on failure). Decided on 6 Tier-3 spikes + 3 research docs (kernel 7.0, `353cdc52`): in-band lossless foreclosed 3 ways; proxy proven agent-light both directions. **Amends whitepaper §7/§8** (two mechanisms → one); **supersedes** in-band kTLS-on-own-socket as v1 — retained as a post-v1 optimization tracked in **#231** (restart-survival + 1-socket density). J-SEC-003 / GH #26 (folds #222) | Accepted |
...
| 0104 | **Historical VM-only replacement boundary** — established the valid #284 physical-identity problem and fresh VM execution evidence, but made `WorkloadLifecycle` choose identity semantics from `WorkloadDriver::Vm(_)` while preserving same-ID Exec. The user rejected that application boundary on 2026-09-13. ADRs 0105, 0106, 0108 and 0109 replace its bundled identity/cleanup/consumption/handoff choices only after final user disposition. | Accepted historical decision; rejected as PR #292 merge authority; corrective supersession unresolved after final review |
| 0105 | **Driver-neutral physical allocation identity** — `WorkloadId` remains stable and one `AllocationId` names one physical execution for every driver, including legacy Exec until GH #293 removal. Exact action/allocator/persistence contracts live in the feature delta. GH #284 | User-ratified; iteration-2 technical check passed; complete DESIGN final verdict `CHANGES_REQUESTED` |
| 0106 | **Successor creation does not wait for predecessor cleanup** — exact-old cleanup completion/failure cannot block or roll back the fresh successor. Exact handoff and cleanup-failure contracts live in the feature delta. GH #284 | User-ratified; iteration-2 decision check passed; F-06 wording corrected after final review |
...
| 0114 | **Embed first public ingress in `overdrive serve`** — one selected node/process/ServerHandle owns the gateway and public-listener lifecycle; not an Allocation, daemon or external proxy | Accepted |
| 0115 | **Singleton Public Route Set aggregate** — one optional Route entity atomically owns first-slice cardinality/conflict; stable `(WorkloadId, port, tcp)` target; public exposure remains outside Service intent | Accepted |
| 0116 | **One coherent Gateway Application admission owner** — the active owner alone stages demand, publishes whole Route/frontend/certified-key generations, drains retained users and owns public-listener admission; singleton Route cardinality remains ADR-0115. | Accepted |
| 0117 | **Producer-neutral Public Certified-Key Custody** — one protected current credential boundary shared by manual input and the later GH #57 Workflow producer. | Accepted |
| 0118 | **Existing cgroup-BPF Dataplane selects gateway backends** — the gateway supplies one exact Service Frontend; BPF alone performs Maglev selection and destination rewrite. | Accepted |
| 0119 | **Dedicated Gateway SVID identity lifecycle** — one non-allocation slot owns issue/audit/renew/restart/drain-ordered drop without fabricating Allocation identity. | Accepted |
| 0120 | **Public listener requires TLS 1.3** — rustls accepts only TLS 1.3 with the custody-owned Web-PKI identity. | Accepted |
| 0121 | **Separate redacted operator gateway status** — ID-scoped application/custody plus independent hydration projection over operator mTLS; no public response-state or secret/receipt detail. | Accepted |
| 0122 | **Distinct Public Certified-Key Custody domain boundary** — Public Certified Key aggregate remains separate from Route, operator HTTPS, internal SPIFFE and allocation IdentityMgr; lifecycle mechanics are owned elsewhere | Accepted |
| 0123 | **Dedicated Gateway Identity Slot/lifecycle** — non-allocation subject, issue/audit/renew/restart/drop and sole gateway-client consumer; allocation identity unchanged | Accepted |
| 0124 | **Service Dataplane selection-receipt ownership** — transient cookie+ServiceKey cgroup-BPF receipt remains with the selector; gateway observes but never owns/selects | Accepted |
| 0125 | **Derived Gateway Frontend Demand lifecycle** — exact staged/current/draining frontend union, staged-before-swap and drain-before-withdraw; no persisted demand truth | Accepted |
| 0126 | **State-based public-ingress ownership** — aggregates, reconcilers and derived views reuse Intent/Observation/View boundaries; no ES/CQRS/event log/new store | Accepted |
| 0127 | **Separate public-certificate acquisition from runtime consumption** — manual input first and ACME #57 later publish through one producer-neutral Public Certified Key boundary; listener/Route forwarding stays acquisition-independent | Accepted |
| 0128 | **Use existing cgroup-BPF Service dataplane for gateway upstream** — exact Service Frontend enters ancestor connect4/Maglev selection; demanded Path-A is TEACHed; XDP wire path unchanged and no userspace LB | Accepted |
| 0129 | **Keep Gateway Application and Service Dataplane generations independent** — no false unified transaction; each connect correlates actual BPF selection through its transient receipt | Accepted |
| 0130 | **Separate public TLS from gateway-SVID workload mTLS** — Web-PKI terminates at the gateway, then a dedicated gateway identity pins the exact BPF-selected workload peer; three credential domains remain distinct | Accepted |
| 0131 | **Gate public listener without redefining workload/Service readiness** — TCP/443 admission depends only on gateway-owned prerequisites and never changes Allocation Running, Service Stable, Backend Eligibility, BPF Hydrated or operator HTTPS meaning | Accepted |
| 0132 | **Preserve the last usable Public Certified Key during replacement** — prove/build/seal/persist before Current publication and write status last; pre-publication failure retains the prior generation. | Accepted |
| 0133 | **Demand-gate Path-A Service-map teaching** — ServiceMapHydrator teaches only exact Staged/Current/Draining Gateway Frontend Demand and acknowledges the applied revision before promotion. | Accepted |
| 0134 | **Gateway-connect selected-backend receipt** — correlate one registered `(SO_COOKIE, ServiceKey)` connect with transient `Selected(BackendId) \| NoBackend` and clean it on every return. | Accepted |
| 0135 | **Commit-gated backend-identity publication** — reserve identity, commit the outer Service-map swap, then expose immutable BackendId-to-applied-workload identity under one Dataplane guard. | Accepted |
| 0136 | **Exact-peer gateway-client mTLS** — accept only the workload SPIFFE URI SAN resolved from the receipt-selected BackendId. | Accepted |
| 0137 | **Public HTTP/1.1 only** — Hyper serves one explicit HTTP version; HTTP/2 and HTTP/3 remain disabled. | Accepted |
| 0138 | **Canonical-authority plus raw-path Route selection** — exact SNI/Host equality and raw Exact/segment-prefix path determine the singleton Public Route. | Accepted |
| 0139 | **Explicit public proxy-header policy** — remove hop-by-hop/untrusted forwarding fields and author canonical Forwarded/Via provenance. | Accepted |
| 0140 | **Fixed finite public runtime limits** — one private first-slice policy bounds admission, parsing, bodies, cleanup and deadlines without operator knobs. | Accepted |
| 0141 | **Stream public proxy bodies with backpressure** — preserve body octets without whole-body memory or disk buffering. | Accepted |
| 0142 | **One upstream attempt per public request** — no retry, replay, hedge or alternate-backend attempt. | Accepted |
| 0143 | **Service Dataplane Applied Backend Identity Association** — receipt BackendId resolves only through the Dataplane-owned readable Applied association; gateway/address indexes cannot publish identity | Accepted |
...
`cgroup_connect4` rewrite → kTLS-TX blocking-splice forward + zero-copy splice
return; inbound nft-TPROXY + `IP_TRANSPARENT` → server-mTLS → kTLS-RX splice
deliver). All four steady-state pumps are agent-light zero-copy `splice`
(ENCRYPT/TX = blocking splice into kTLS-TX per ADR-0069's 2026-08-26 amendment,
which replaced the interim `write_all` copy; DECRYPT/RX = non-blocking splice out
of kTLS-RX).
...
A **two-ADR refactor of the reconciler primitive** (ADR-0087 precursor then
ADR-0086), sequenced precursor-first. ADR-0087 consolidates restart authority —
`WorkloadLifecycle` becomes the **sole** restart authority (one `restart_counts`
budget spans crash **and** liveness, the kubelet shape), dissolving the
`ServiceLifecycle`→`WorkloadLifecycle` cross-read at its root; `ServiceLifecycle`
demotes to readiness/membership + liveness-terminate. ADR-0086 then moves reconciler
hydration off the central ~1100-line control-plane `match` free fns **onto the
reconciler impls**, extracts the impls + 3 dispatch enums into a NEW
...
golden + full suite green); ADR-0087 is a deliberate, cause-carrying change on the
liveness terminal reason only. DELIVER complete (**7 steps** 01-01…02-05 all
COMMIT/PASS; full workspace **2777/2777 green**; adversarial Opus review **APPROVED
after one remediation pass**; **mutation `transition_reason.rs` 6/6 = 100%** +
manual temp-break flip-proofs; DES integrity exit 0). Evolution record:
`docs/evolution/reconcilers-own-hydration-evolution.md`.

| Component | Path | Disposition |
|---|---|---|
| `overdrive-reconcilers` crate (`crate_class = "adapter-host"`) — 8 reconciler impls + 3 dispatch enums (`AnyReconciler`/`AnyState`/`AnyReconcilerView`) + `service_lifecycle` + per-reconciler `*State`/`*View` + pure helpers + the moved `hydrate_*` bodies; depends only DOWN on core | `crates/overdrive-reconcilers/` | NEW |
| 4 driven read-port traits — `ListenerFacts::fact_for` (async), `ServiceVipView::assigned_vip` (async), `WorkflowLiveSet::live_instances` (sync), `HeldSvidView::held_snapshot` (sync); **no `RestartBudgetView`** (ADR-0087 removed the cross-read) | `crates/overdrive-core/` | NEW |
| `HydrationContext<'_>` + `HydrateError` (`IntentRead`/`ObservationRead`/`IntentDecode`, `#[from]`→`ConvergenceError`); `Reconciler::hydrate_desired`/`hydrate_actual` async trait methods; `HeldSvidFacts` relocated to core (crosses the `HeldSvidView` signature) | `crates/overdrive-core/` | NEW / EXTEND / RELOCATE |
| `StoppedBy::LivenessProbe` (fieldless rkyv **tail** variant, discriminant 5) + `is_liveness_killed` dual-field predicate + cause-aware exhaustion terminal (`ServiceFailed{LivenessProbeFailed}` ≠ `BackoffExhausted`); schema-evolution golden `FIXTURE_LIVENESS_PROBE_V1` (existing fixtures untouched) | `crates/overdrive-core/src/transition_reason.rs` | EXTEND (ADR-0087) |
| `ReconcilerRuntime` builds a `HydrationContext` per tick + calls `AnyReconciler::hydrate_*`; implements **no** read-port (former `RestartBudgetView` gone); central `hydrate_*` free fns + ~11 helpers DELETED (single-cut) | `crates/overdrive-control-plane/src/reconciler_runtime.rs` | EXTEND (+ single-cut delete) |
| `ServiceLifecycle`→`WorkloadLifecycle` cross-read (`restart_status_for_alloc` CALL in `hydrate_service_alloc_facts`) + `ServiceAllocFact.{restart_count,restart_spec}` + `RestartReason` + `Action::RestartAllocation.reason` DELETED; the `restart_status_for_alloc` **method** KEPT (4 live streaming callers) | `crates/overdrive-control-plane/` | DELETE (ADR-0087; method kept) |
| 3 control-plane read-port impls (`ListenerFactStore`→`ListenerFacts`, `WorkflowEngine`→`WorkflowLiveSet` narrow-view-only, `IdentityMgr`→`HeldSvidView`) + dataplane `PersistentServiceVipAllocator`→`ServiceVipView` | `crates/overdrive-control-plane/`, `crates/overdrive-dataplane/` | EXTEND |
| 4 `Sim*` read-port impls (`SimListenerFacts`/`SimServiceVipView`/`SimWorkflowLiveSet`/`SimHeldSvidView`) — hydration boundary now DST-injectable | `crates/overdrive-sim/` | NEW |
...
> `overdrive-core → overdrive-reconcilers` is **dev-dep only** — the reconciler
> **contract** (trait + read-ports + `HydrationContext`) stays in core, the **impls**
> move to the adapter crate. Pure-sync `reconcile` unchanged ⇒ DST replay survives;
> the Sim read-ports are a net injectability gain (the hydration boundary was
> previously invisible to the harness). Enum-collapse (#272) is an out-of-scope
> pragmatic-cut boundary (ADR-0086 D2), not a deferral introduced here.
...
| **`HostMtlsEnforcement` adapter** | EXTEND `overdrive-dataplane` (OQ-2 resolved 2026-06-12; `overdrive-host` ruled out for `#![forbid(unsafe_code)]`) | `adapter-host` | The production proxy, BIDIRECTIONAL (F3). OUTBOUND: `cgroup_connect4`-rewrite intercept, lossless capture, rustls CLIENT handshake on leg B, kTLS arm, blocking-`splice` forward pump into leg B's kTLS-TX (agent-light zero-copy; ADR-0069 2026-08-26 amendment — replaced the interim `write_all` copy, which itself replaced the retired sockmap egress-redirect), `splice` return pump out of kTLS-RX (agent-light zero-copy). INBOUND: `nft`-TPROXY + `IP_TRANSPARENT` intercept, `getsockname` orig-dst, rustls SERVER handshake on leg C (present server SVID + `WebPkiClientVerifier` verify client), kTLS-RX arm, `splice`-to-server deliver pump (agent-light) + blocking-`splice` response pump into leg C's kTLS-TX; fail-closed on `nocert`/`wrongca`. Consumes `IdentityRead`. Extends the established userspace eBPF crate (already hosts `EbpfDataplane`; `unsafe` allowed, `aya` + BPF `build.rs` present). |
...
| **New BPF programs** | `overdrive-bpf` (extend) | (BPF) | OUTBOUND: `cgroup_connect4` *transparent-mTLS variant* (intercept rewrite — extends the existing `cgroup_connect4_service` shape). INBOUND (F3): `nft`-TPROXY + `IP_TRANSPARENT` listener (server intercept — the mirror of `connect4`; orig-dst via `getsockname`). NO psock/verdict/sockmap on ANY leg — the forward `sockops` (ESTABLISHED detect) + `sk_skb/stream_verdict` (egress-redirect leg F → leg B) programs the original design added were RETIRED 2026-06-13 with the sockmap; the forward encrypt is now a userspace blocking `splice` into kTLS-TX with no kernel-side program (ADR-0069 2026-06-13 + 2026-08-26 amendments). Every leg is a plain kTLS socket (leg B return / leg C deliver / forward + response all use plain `splice`). |
...
| Reliability (pump supervision, F6) | The `splice` pumps are supervised: `Stalled` (no bytes-spliced progress for `pump_stall_deadline` (30 s) with a record pending) → the worker tears the connection down (fail-closed reset), never degrades to a userspace copy loop | Tier-3: inject a stalled pump → `liveness == Stalled` → worker teardown → `Gone`, no fd/kTLS leak (no sockmap state exists post-2026-06-13) |
...
| Performance (agent-light, zero-copy) | All four pumps agent-light zero-copy `splice`: forward/response = BLOCKING splice into kTLS-TX (`tls_sw_sendmsg`/`MSG_SPLICE_PAGES` encrypts in-kernel); return/deliver = non-blocking splice out of kTLS-RX (~1/record). No userspace crypto, no plaintext copy, in any direction. *(2026-08-26 amendment: forward moved sockmap-egress-redirect → `write_all` copy → blocking splice; `splice` into kTLS-TX does NOT lose records — the loss class was non-blocking `MSG_DONTWAIT` delivery.)* | strace: forward pump issues `splice` only (no per-byte read/write); ~1 splice/record every direction; `eagain=0` on the forward pipe→kTLS-TX splice |
...
declared usable, ONE composed sentinel (`ProbeSentinel::KtlsArmRoundTrip`) verifies
the kTLS arm + agent-light forward-encrypt round-trip — a sentinel rustls TLS 1.3
handshake arms kTLS-TX/RX on a loopback leg, the forward encrypt pump (the EXACT
production forward primitive — a BLOCKING `splice` into kTLS-TX, ADR-0069 2026-08-26
amendment) moves one sentinel record through it ENCRYPTED, and the sentinel peer's
kTLS-RX reconstructs the exact plaintext via a single `tls_sw_splice_read`; the
reader leg also exercises the kTLS 0.5-RTT early-data drain. On probe failure the
node refuses to start with `health.startup.refused`. *(The obsolete
`ForwardEgressRedirect` and `ArmingOrderEinval` sub-sentinels were dropped 2026-06-13
with the sockmap — there is no sockmap insert, no egress redirect, and no
`SOCKMAP`-vs-`TCP_ULP` ordering on any path now.)* This is not a convention but a
compile-and-CI-enforced contract (subtype + structural + behavioural layers per
principle 12).
...
`image_type`, no caller-supplied Landlock path list, no `memory_max` and no `rlimit_fsize` is
...
| **C-4** — US-VM-7 names the paths CH auto-derives and omits launch dependencies that must be explicit | CH auto-derives Landlock rules for `--kernel` / `--disk` / `--serial file=` / `--api-socket` but **not** for the vsock socket it binds itself; ADR-0089's later selected-TAP attach additionally reads `/sys/class/net/<tap>` after Landlock is enabled | `VmRunDir::landlock_grant()` remains the fixed `access=rw` directory producer. **Proposed 2026-09-11, pending review:** `VmConfig::landlock_rules()` becomes the sole ordered rule-set composer: for `network: Some`, exact selected-TAP leaf `access=r`, then run dir `access=rw`; for `None`, the legacy run-dir-only rule. No caller path/access input and no host-side formatter |
...
**Proposed Landlock composition shape (2026-09-11; pending independent
review).** The public signatures stay unchanged: `LandlockRule::{to_rule_arg,
path}`, `VmRunDir::landlock_grant`, and `VmConfig::landlock_rules`. Inside
`overdrive_core::vm::config`, `LandlockRule` gains only a private
`LandlockAccess { ReadOnly, ReadWrite }` discriminator and
`VmNetworkAttachment` gains only private
`tap_sysfs_landlock_grant(&self) -> LandlockRule`. That helper has no path or
access parameter and derives the canonical sysfs leaf from the same `tap` value
the VMM renders in `--net`. The existing run-dir method constructs
`ReadWrite`; the private TAP method constructs `ReadOnly`; and
`VmConfig::landlock_rules` alone composes the complete deterministic sequence.
`CloudHypervisorVmm` deletes its parallel
`network_tap_sysfs_landlock_rule` formatter and renders only those returned
values. This is the exact minimum reconciliation; full invariants, rejected
alternatives, test transitions, lifecycle-gate non-change and Reuse Analysis
are in ADR-0082's proposed 2026-09-11 amendment.

The amendment's Reuse Analysis classifies every touched return-only boundary
with the exact declaration `CONTRACT_SHAPE: pure-function.` — selected-TAP
identity flow, `LandlockRule::to_rule_arg`, both fixed producers,
`VmConfig::landlock_rules`, and `CloudHypervisorVmm::build_confined_command`.
The unchanged TAP/network/lifecycle effect family is explicitly
`CONTRACT_SHAPE: bounded-change.` over its existing per-allocation universe.
The executable handoff is likewise exact: retain
`hypervisor_runs_bounded_nonroot_and_landlock_confined`, rename the stale
vsock-only test to
`networked_vm_landlock_rules_are_exact_tap_read_then_run_dir_write`, and place
`/// CONTRACT_SHAPE: bounded-change.` on each; add the core property
`landlock_rules_are_exact_and_ordered_for_network_presence` with
`/// CONTRACT_SHAPE: pure-function.`. ADR-0082 owns the non-overlapping argv,
path-scope and core-composer oracles in full.
...
**Mechanism update — 2026-08-23 (ADR-0084 §4, GH #266): the driving mechanism was
unified onto the generic Piece A cadence hook; the value and the not-tunable
property above are UNCHANGED.** The cadence is no longer a hardcoded submission
inside `spawn_convergence_loop`. `VmReclamation` now declares its own
`resync_schedule() → Some(ResyncSchedule { period: VM_RECLAMATION_SWEEP_INTERVAL,
scope: ResyncScope::LocalNode })`, and `VM_RECLAMATION_SWEEP_INTERVAL` relocated
to `overdrive-core` (beside `VmReclamation`) — only its home crate changed. The
generic cadence phase picks the schedule up at registration and submits
`node/<local_node_id>` every 30 s via `broker.submit` + the core
`resolve_scope(LocalNode, node_id)` — behaviourally identical to the deleted sweep
(same target, same `now + period` first-fire). The loop now names no reconciler and
bakes no cadence constant, and `vm-reclamation` is the hook's first live consumer.
...
submission and leaves the generalisation where Titan put it. *(Superseded
2026-08-23, ADR-0084 §4 / GH #266: the generalisation has since been founded —
the generic Piece A cadence hook — and `vm-reclamation` is unified onto it as its
first live consumer via `resync_schedule`, replacing the one hardcoded loop
submission; see the Mechanism update above.)* § *System
...
| `KernelImage::validate`, `MemoryPlan::derive`, `DiskAttachment::to_disk_arg`, `VmConfinement::seccomp_arg`, `VmConfig::rlimit_fsize`, `VmConfig::landlock_rules` | **pure-function** | ∅ | Unit + proptest; `landlock_rules` has no port and mutates nothing. **Proposed 2026-09-11, pending review:** its exact return is `[run-dir rw]` for `network: None` and `[selected-TAP sysfs r, run-dir rw]` for `network: Some`, with no third entry. `rlimit_fsize` is pure only because `RootfsPlan` carries `master_bytes` captured at construction — without that field it is a `stat(2)` wearing a pure signature. The historical Slice-01 deferral is closed by US-VM-7; the new TAP leaf reconciles ADR-0089's later attachment without adding an effect. |
...
C4Component
    ...
    Container_Boundary(serve, "overdrive serve (one OS process)") {
        ...
        Component(cfg, "VmConfig builder", "overdrive-core values", "Derives ordered explicit Landlock grants, memory plan, rlimit and disk args")
    ...
    System_Ext(rundir, "/run/overdrive/vm/<alloc>/", "tmpfs; exclusive; rw Landlock grant; networked proposal adds separate selected-TAP sysfs r grant")
...
| **Security — confidentiality** | A compromised hypervisor **process** attempts to open a sibling VM's disk or inspect another host interface | The allocation run-dir grant remains exclusive and `rw` (P5: sibling path denied). **Proposed 2026-09-11, pending review:** a networked VM gains only `access=r` on `/sys/class/net/<its-selected-tap>`, never the parent, another TAP, an alternate path or write access. **Scoped to the process, not the guest**, and carrying P5's own caveat: this is the same path set CH was given, not a byte-copy of CH's internal ruleset — CH exposes no way to prove the latter. P5 tested a host-side process under the identical path set; it did not test a guest, and a guest reaches host paths only through virtio devices |
...
| 31 | `spawn_convergence_loop` (`lib.rs:2427-2477`) + `EvaluationBroker` (`eval_broker.rs:59`, `submit` at `:85`) | waking reconcilers | **EXTEND — re-verdicted at iteration 3** | Two obligations, and the first was the only one the Bar-1 draft had. **(a) The wake after a row write:** `execute_reclaim_allocation` submits the **four** evaluations the exit observer submits per exit — `workload_lifecycle` (`worker/exit_observer.rs:234`), `backend_discovery_bridge` (`:254`), `service_lifecycle` (`:295`) and **`svid_lifecycle` (`:318-320`)**; without the fourth, `DropSvid` never fires and **the node keeps the dead allocation's leaf private key** (`svid_lifecycle.rs:316-317`, `:506-513` — ADR-0067 O2). **(b) NEW, and Bar 2 does not supply it for free: `vm-reclamation` must be resynced every `VM_RECLAMATION_SWEEP_INTERVAL` (30 s).** The broker is purely event-driven with no bootstrap sweep and `has_work` only *re*-enqueues a reconciler that already ticked, so **nothing in tree would ever tick this one**. As of the ADR-0084 §4 / GH #266 unification (2026-08-23) this cadence is supplied by `VmReclamation`'s own `resync_schedule` (30 s / `LocalNode`) through the generic Piece A cadence hook — NOT a hardcoded `spawn_convergence_loop` submission — with `VM_RECLAMATION_SWEEP_INTERVAL` relocated to `overdrive-core`; behaviourally identical (same target, same first-fire), and `vm-reclamation` is the hook's first live consumer. The generalisation the sweep once foreshadowed for [#197](https://github.com/overdrive-sh/overdrive/issues/197) (#198 / #199 / #234) is now that Piece A hook — see § 105a.8 for the three rejected alternatives the ratified cadence was chosen against |
...
| The `--landlock-rules` flag is rendered only by `CloudHypervisorVmm`, while complete rule values/order come only from `VmConfig::landlock_rules`; `LandlockRule` has no public constructor | Existing `xtask dst-lint` clause keeps the flag literal at the one host argv site; Rust privacy confines rule construction to `overdrive_core::vm::config`; pure/host argv tests assert exact count, value, access and order. **Proposed 2026-09-11:** the sole public rule producer remains `VmRunDir::landlock_grant`, plus exactly one private `VmNetworkAttachment::tap_sysfs_landlock_grant`; the adapter-local formatter is deleted | `crates/overdrive-core/src/vm/config.rs`; `crates/overdrive-host/src/vmm.rs`; exact contract in ADR-0082 proposed amendment |
...
**Proposed Landlock contract reconciliation (2026-09-11; pending independent
architecture review).** The accepted TAP attach introduced one necessary CH
read that this section and ADR-0089 did not carry into ADR-0082: under
Landlock, CH reads `/sys/class/net/<selected-allocation-tap>` to obtain TAP
flags. Removing only that grant from the real production path yields
`Permission denied`, no `Running`, and `VmGuestExitUnreported` after 90.86 s.
For a networked VM the exact explicit-rule contract is therefore proposed as
TAP sysfs leaf `access=r` first, allocation run directory `access=rw` second;
a non-networked VM retains the single run-directory rule. The existing
`VmConfig::landlock_rules()` is the sole complete composer; the adapter-local
formatter is removed. No public API, TAP identity, provisioning, lifecycle
gate, persistence, cleanup/security subsystem or guest-wire behavior changes.
See ADR-0082 and ADR-0089's proposed 2026-09-11 amendments.
...
2. **CH net attach** (ADR-0089): `Vmm` prepends `ip netns exec <ns>` to the
   ...
   already-sanctioned CH subprocess). **Proposed pending review:** the same
   selected TAP identity also produces the exact read-only
   `/sys/class/net/<name>` Landlock rule through `VmConfig::landlock_rules`,
   before the existing run-directory read-write rule; `Vmm` does not format a
   second rule value itself.
...
### Public-ingress gateway application architecture (ADR-0116–0121/0132–0142, Accepted 2026-09-13)

Application DESIGN composes the System and Domain choices into one active,
in-process public gateway. `overdrive-control-plane` remains the sole
`serve`/`ServerHandle` composition owner; one non-deployable
`overdrive-gateway` adapter-host library owns Route application, protected
public-key consumption and bounded TLS/HTTP runtime. Existing
ServiceMapHydrator/EbpfDataplane, CA/SVID and workload mTLS owners remain
authoritative for their effects.

The application path is: top-level Route deploy discrimination → one coherent
Route/exact-Service-Frontend/opaque-certified-key admission generation →
staged/current/draining frontend demand → registered socket-cookie +
ServiceKey cgroup-BPF selection and actual BackendId receipt → immutable
applied workload identity → dedicated gateway-SVID exact-peer mTLS →
bounded Hyper HTTP/1.1 streaming. XDP wire ingress is unchanged; the gateway
never watches/enumerates/selects backends, mutates raw BPF maps or fabricates
an AllocationId.

Each independently reversible choice has one concise Accepted decision record:

| ADR | One decision |
|---|---|
| [0116](adr-0116-public-ingress-route-and-gateway-application-owner.md) | One active owner for coherent Gateway Application admission/listener lifecycle |
| [0117](adr-0117-producer-neutral-public-certified-key-custody.md) | Producer-neutral protected Public Certified-Key Custody |
| [0118](adr-0118-existing-cgroup-bpf-gateway-backend-selection.md) | Existing cgroup-BPF Dataplane remains the only gateway backend selector |
| [0119](adr-0119-dedicated-gateway-svid-identity-lifecycle.md) | Dedicated non-allocation Gateway SVID identity lifecycle |
| [0120](adr-0120-public-listener-tls13-only.md) | TLS 1.3 only on the public listener |
| [0121](adr-0121-redacted-operator-gateway-status.md) | Separate redacted operator gateway-status projection |
| [0132](adr-0132-preserve-last-usable-public-certified-key-replacement.md) | Preserve-old Public Certified-Key replacement transaction |
| [0133](adr-0133-demand-gated-path-a-service-map-teach.md) | Demand-gated Path-A Service-map teaching |
| [0134](adr-0134-gateway-connect-selected-backend-receipt.md) | Per-connect BPF-selected BackendId receipt |
| [0135](adr-0135-commit-gated-backend-identity-publication.md) | Commit-gated applied backend-identity publication |
| [0136](adr-0136-exact-peer-gateway-client-mtls.md) | Receipt-selected exact-peer gateway-client mTLS |
| [0137](adr-0137-public-http11-only.md) | Public HTTP/1.1 only |
| [0138](adr-0138-canonical-public-route-match-key.md) | One canonical authority/raw-path Public Route match key |
| [0139](adr-0139-public-proxy-header-policy.md) | Explicit public proxy-header policy |
| [0140](adr-0140-fixed-finite-public-runtime-limits.md) | Fixed finite first-slice public runtime limit policy |
| [0141](adr-0141-stream-public-proxy-bodies-with-backpressure.md) | Streaming request/response bodies with backpressure |
| [0142](adr-0142-single-upstream-attempt-per-public-request.md) | One upstream attempt per public request |

The exact implementation contract is owned only by the feature delta:
[component decomposition](../../feature/public-ingress-gateway/feature-delta.md#wave-design--ref-application-component-decomposition),
[driving/driven ports](../../feature/public-ingress-gateway/feature-delta.md#wave-design--ref-exact-driving-and-driven-ports),
[custody and persistence](../../feature/public-ingress-gateway/feature-delta.md#wave-design--ref-persistence-custody-and-redaction),
[runtime/status](../../feature/public-ingress-gateway/feature-delta.md#wave-design--ref-gateway-application-status-and-http-contract),
[identity lifecycle](../../feature/public-ingress-gateway/feature-delta.md#wave-design--ref-gateway-identity-and-lifecycle-integration),
and [DISTILL handoff/outcomes](../../feature/public-ingress-gateway/feature-delta.md#wave-design--ref-application-verification-and-full-stack-handoff).
The sole detailed C4 source is
[Canonical public-ingress C4](c4-diagrams.md#public-ingress-gateway-canonical-c4).

Manual operator-supplied Web-PKI material is the only first-slice public
certificate producer; test environments use a test CA. GH #57 may later add
a Workflow producer at the same custody install boundary, without replacing
Route, listener or TLS consumption ownership. The complete production
composition remains recurring Tier-3 boundary `AT-PIG-E2E-1`, distinct from
the root operator example and the point-in-time EDD expectation. Typed
outcomes and their reused CA/SVID/dataplane relations are registered in
`docs/product/outcomes/registry.yaml`.

---

## Driver-neutral allocation replacement correction (GH #284; ADR-0105/0106/0108/0109)

**Date:** 2026-09-13. **Status:** the user explicitly ratified P-105-1 through
P-105-7, selecting P-105-4A, P-105-5A and P-105-6A. The user-capped final
review iteration 2 returned `CHANGES_REQUESTED`; its bounded F-03…F-06
documentation findings were remediated without changing the ratified
mechanism, and no third review is authorized. Explicit user disposition is
required before this becomes implementation authority. PR #292 is not
mergeable before acceptance, re-DISTILL and re-DELIVER.

[ADR-0105](adr-0105-driver-neutral-allocation-replacement-identity.md) records
one driver-neutral physical-allocation identity: `WorkloadId` remains stable
and every replacement execution receives a distinct `AllocationId`, including
legacy Exec until GH #293 removes it.

[ADR-0106](adr-0106-successor-creation-does-not-wait-for-predecessor-cleanup.md)
records the independently reversible no-wait choice: after the approved
predecessor handoff, successor creation does not wait for exact-old cleanup,
and old cleanup failure cannot select, consume, block or roll back the
successor identity.

[ADR-0108](adr-0108-durable-reservation-consumes-successor-allocation-identity.md)
records the independently reversible consumption choice: durable View
reservation consumes the successor identity before dispatch, including when a
prelaunch failure leaves an unused gap.

[ADR-0109](adr-0109-replacement-requires-terminal-predecessor-handoff.md)
records the independently reversible lifecycle handoff: only an accepted
numeric-current `Failed` or `Terminated` predecessor is eligible;
`Draining` is insufficient.

[ADR-0107](adr-0107-successor-identity-consumption-follows-successor-owned-effect.md)
is withdrawn before acceptance because its successor-owned-effect boundary is
the opposite of ratified P-105-5A.

The canonical
[feature delta](../../feature/vm-recreation-allocation-id-reuse/feature-delta.md)
owns the exact action fields, ratified private signatures, View and publication
contracts, lifecycle gates, cleanup ordering and the dated P-105-1…P-105-7
ratification record. No reservation-phase, cleanup-retry or driver-policy API
is authorized. There is no unresolved substantive decision in this corrective
DESIGN; disposition of the final `CHANGES_REQUESTED` review remains the DESIGN
governance gate.

Detailed relationships live only in
[C4 diagrams](c4-diagrams.md#driver-neutral-allocation-replacement-gh-284-corrective-proposal).
Greptile's network-leak claim remains an unproven hypothesis and adds no
...
rejected its VM-only application boundary on 2026-09-13. It remains the
historical record for the reproduced VM artifact alias and the rejected
VM-`StartAllocation`/Exec-same-ID solution. It is not current merge authority.

The user-ratified ADR-0105, ADR-0106, ADR-0108 and ADR-0109 replace that
boundary only after explicit user disposition of the final review. ADR-0107 is
withdrawn. See
[ADR-0104](adr-0104-vm-recreation-fresh-allocation-identity.md) for the
historical decision and
[the corrective feature delta](../../feature/vm-recreation-allocation-id-reuse/feature-delta.md)
for current proposed contracts.
## Changelog
...
| 2026-09-13 (DESIGN Application ADR-taxonomy remediation / accepted; user-approved) | **Public-ingress Application choices now occupy one independently reversible decision per ADR across ADR-0116–0121 and ADR-0132–0142.** ADR-0117–0120 retain respectively custody ownership, existing cgroup-BPF selection, Gateway SVID lifecycle and TLS1.3-only decisions; ADR-0132–0142 separately record preserve-old replacement, demand-gated TEACH, selected-backend receipt, commit-gated identity publication, exact-peer mTLS, HTTP/1.1, Route matching, proxy headers, finite limits, streaming and one upstream attempt. Exact API remains only in the feature delta, C4 only in `c4-diagrams.md`, and executable obligations in DISTILL. No behavior, API or test-scope change. The complete DESIGN was explicitly approved by the user on 2026-09-13. — Morgan. |
| 2026-09-13 (DESIGN System ADR-scope remediation / accepted; user-approved) | **Public-ingress System choices split into ADR-0114 and ADR-0127–0131 under the one-decision-per-ADR rule.** ADR-0114 now owns only embedded single-node placement; the new records separately own acquisition-neutral public-certificate consumption, cgroup-BPF packet entry, independent Gateway/BPF generations, the public-to-internal trust transition and listener-gate ownership. The System brief/feature embedded Mermaid copies were removed; the canonical L1/L2/L3 source lives only in `c4-diagrams.md`. Route ADR-0115, Domain ADR-0122–0126/0143 and exact Application `[REF]` ownership remain external to those System decisions; Application decisions are ADR-0116–0121/0132–0142. No runtime semantic change. The complete DESIGN was explicitly approved by the user on 2026-09-13. — Titan. |
| 2026-09-13 (DESIGN application stage revised / accepted; user-approved) | **Public-ingress Application decisions were initially grouped into ADR-0116–0121 under the one-decision-per-ADR rule and are further separated by the taxonomy remediation above.** Exact signatures and implementation contracts moved to the feature delta; detailed C4 remains only in `c4-diagrams.md`; this brief summarizes and links. The active path retains preserve-old manual certified-key custody, commit-guarded BPF selected-identity publication, dedicated gateway-SVID exact-peer mTLS, fixed finite TLS1.3/HTTP1.1, redacted operator status, no userspace LB and recurring Tier-3 `AT-PIG-E2E-1`. The complete DESIGN was explicitly approved by the user on 2026-09-13. — Morgan. |
