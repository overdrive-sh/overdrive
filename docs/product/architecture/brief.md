...
| TRC-ARCH-002 route-port reuse: `ServiceAllocFact.terminal: Option<TerminalCondition>` copied from current and `driver_route_present: bool` from one service-hydration route snapshot; no `ServiceLifecycleState`/View field and no sixth port | `overdrive-reconcilers` | EXTEND — process-local input only |
...
4. **The intercept-gate flip** (ADR-0089 D6): the `DriverType::Exec` gate on
   ...
   **2026-08-31 failure-unwind amendment:** after slot assignment, any C3/VM-TAP
   provision error attempts netns → owned host-stranded TAP → host-veth/route →
   resolver cleanup without early exit, releasing the slot only after total
   success and retaining the provision error as primary in Failed. Same-id
