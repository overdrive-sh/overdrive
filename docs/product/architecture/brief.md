...
| 0086 | Reconcilers own async intent/observation hydration through `HydrationContext`; implementations live in `overdrive-reconcilers`, with narrow core read-ports breaking the Cargo cycle. **2026-08-31 TRC-ARCH-001 amendment:** complete port set 4→5 with synchronous `AllocDriverRouteView::routed_allocations`; production binding is the existing process-local `AllocDriverIndex` underlying map, exposed as keys only. `HydrationContext.alloc_driver_routes` feeds target-intersected `WorkloadLifecycleState.routed_allocations`; no durable route, reconciler-View cross-read, new component, or crate edge. | Accepted (amended 2026-08-31) |
...
detached completion future exists. The exact outcome table is feature delta
R4a; ADR-0048 owns store closure semantics and ADR-0083 owns supervision/route.
