...
| 0048 | rkyv versioned envelopes at redb persistence boundaries; intent refuses unreadable SSOT while ordinary observation reads log + skip. **2026-08-31 allocation-lifecycle amendments:** the sole compound AllocStatus writer atomically self-heals a malformed/unknown-future prior current row and appends a typed `Unreadable` predecessor occurrence; `Absent` means no bytes existed, and no generic current-only writer remains. Its existing local redb blocking closure owns an in-flight compound transaction plus accepted-current subscription send attempt through caller cancellation; acceptance is reconciled from current plus process-local route membership, never a receipt. The public method stays async; no sync facade/runtime lookup/detached async completion. | Accepted (amended 2026-08-31) |
...
| 0086 | Reconcilers own async intent/observation hydration through `HydrationContext`; implementations live in `overdrive-reconcilers`, with narrow core read-ports breaking the Cargo cycle. **2026-08-31 TRC amendments:** complete port set 4→5 with synchronous `AllocDriverRouteView::routed_allocations`; production binding is the existing process-local `AllocDriverIndex` underlying map, exposed as keys only. `HydrationContext.alloc_driver_routes` feeds target-intersected `WorkloadLifecycleState.routed_allocations`; the same snapshot supplies only `ServiceAllocFact::{terminal, driver_route_present}` for liveness convergence. TRC-ARCH-003 adds no port/fact/State field: private hydration filters liveness probes strictly after current `started_at`, and the existing Service View gains one serde-defaulted attempt-marker input. No sixth port, durable route, reconciler-View cross-read, new component, or crate edge. | Accepted (amended 2026-08-31) |
...
| TRC-ARCH-002 route-port reuse: `ServiceAllocFact.terminal: Option<TerminalCondition>` copied from current and `driver_route_present: bool` from one service-hydration route snapshot; no `ServiceLifecycleState` field and no sixth port | `overdrive-reconcilers` | EXTEND — process-local input only |
| TRC-ARCH-003 attempt handoff: `#[serde(default)] ServiceLifecycleView.liveness_attempt_started_at: BTreeMap<AllocationId, UnixInstant>`; changed Running start clears old counter before dispatch; private hydration admits only probes completed strictly after current start | `overdrive-reconcilers` | EXTEND — existing ViewStore input, no receipt/cross-read |
...
serde-defaulted `ServiceLifecycleView.liveness_attempt_started_at` map. On the
first Running tick for a changed `started_at`, Service clears the historical
counter and persists the new marker before action dispatch; hydration supplies
no liveness result unless its completion is strictly later than that start.
Thus a frozen batch may remove the route and restart the same id before the
Service clear: the replacement still emits no Stop from the old probe/threshold
and requires fresh-attempt failures. Non-Running repair retains marker+counter,
so cancellation remains replayable. No broker priority, receipt, new action,
or cross-View read exists.
...
4. **The intercept-gate flip** (ADR-0089 D6): the `DriverType::Exec` gate on
   ...
   **2026-08-31 failure-unwind amendment:** after slot assignment, any C3/VM-TAP
   ...
   three prove absence. The netns name remains the existing boot slot anchor
   until dependent residue is gone; existing adopt/orphan-GC runs before any
   allocation, and release requires all four absences. The provision error
