...
**Date:** 2026-09-06.
**Status:** proposed, user-selected in guided Application/component DESIGN;
...
per-allocation registration boundary. A non-wildcard explicit host is retained.
An omitted HTTP host or `0.0.0.0` targets the VM allocation's provisioned guest
`workload_addr`; the same declaration on Exec/process preserves loopback.
Declared descriptors remain intent and are never rewritten with runtime
addresses. See ADR-0090.
...
**Not applicable:** this extension changes one runtime network destination and
one pre-persistence admission rule. It adds, removes, and moves no lifecycle or
health gate.
...
| `Backend.healthy` | `ServiceLifecycle` readiness branch | Existing readiness threshold is currently met |
...
A closed guest port therefore produces a startup failure while the VM remains
Running. Startup, readiness, and liveness results stay in their own domains.
...
redb, Linux routed TAP/netns, and Cloud Hypervisor; no new dependency, daemon,
transport, or protocol is selected. The repository's existing modular-monolith
