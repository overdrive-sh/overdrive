...
| 0088 | **Guest-stack netns topology — routed two-/30 tap wire + guest addressing** — VM-kind allocs get a persistent tap in the per-workload netns addressed from a second slot-derived /30 (upper half of `WORKLOAD_SUBNET_BASE` `10.99.0.0/16`: `base + 0x8000 + slot*4`; disjoint from the transit /30 by construction under `NET_SLOT_MAX = 4095`); `ip_forward=1` in the netns; host return route to the guest /30 — byte-for-byte the increment-n spike topology (WORKS, no rp_filter/tx-offload toggle). **`workload_addr` = the guest addr** for VM allocs (rides the existing `AllocStatusRowV2` field, no envelope bump) — the only address a peer's leg-S dial / inbound `daddr` match can terminate at. Guest addressing = ONE platform-owned kernel-cmdline parameter (`guest_addr/30`, gateway, dns=responder) applied by `overdrive-init` via the spike-proven ioctls + guest `/etc/resolv.conf` (dial-by-name reused), FAIL-CLOSED (apply-or-EXIT before exec); beacon PL unchanged. Rejected: L2 bridge tap↔veth (unproven, br_netfilter/L2 surface), /32-onlink unnumbered tap, second /16 guest base (escapes the mesh-membership /16), kernel `ip=` (CONFIG_IP_PNP unset; no failure surface), DHCP, a new beacon vsock message. Extends ADR-0071 to guest-stack; realises ADR-0069's staged #222 adapter. GH #222 | Accepted |
| 0089 | **Tap-in-netns provisioning boundary + CH net attach** — the C3 seam (`provision_and_inject_netns`) gains a `DriverPayload`-matched VM branch: pure `VmTapPlan` (tap `ovd-tp-<4hex-slot>`, guest /30, MAC) + four Bar-1 converge steps beside the veth steps (tap create+persist, tap addr, netns `ip_forward`, host return route — return-route ownership = the provisioner, teardown structural); tap creation subprocess-free (`/dev/net/tun` ioctl + netlink `IFLA_NET_NS_FD` netns-move; EXTEND `overdrive-netlink`); `Vmm` prepends `ip netns exec <ns>` to the existing wrapper argv + `--net tap=,mac=` (`VmConfig.netns` becomes CONSUMED; `overdrive-host` stays `#![forbid(unsafe_code)]`); the `DriverType::Exec` intercept gate (`action_shim/mod.rs:1559`) extends to VM-kind (D-MTLS-18 fail-closed inherited). Inbound (peer→guest) topology-settled, BUILD deferred to #257 (no production path can declare a guest listener until the `[vm]+[service]` parse rejection is removed — the #236 dead-mechanism precedent). Rejected: driver-creates-tap, tap fd-passing (`--net fd=`, the evidence-gated refinement), worker `pre_exec` setns, a dedicated Bar-2 reconciler now (#197/#234 rides), `start_alloc`-owned return route. Bar-2 promotion rides #197/#234. GH #222 | Accepted |
...
toggle) proved the shipped `install_outbound_tproxy` rule fires **byte-for-byte
unchanged** over a tap-fed veth. Full narrative:
...
3. **Guest addressing** (ADR-0088): routed two-/30 topology — the guest /30 is
   carved from the SAME `WORKLOAD_SUBNET_BASE` `10.99.0.0/16`, upper half
   (`base + 0x8000 + slot*4`), so mesh-membership tests on the /16 stay
   correct; **`workload_addr` = the guest addr** for VM allocs (rides the
   EXISTING `AllocStatusRowV2` field — no envelope bump). ONE platform-owned
   kernel-cmdline parameter carries `(guest_addr/30, gateway, dns=responder)`;
   `overdrive-init` applies it via the spike-proven ioctls + writes the guest
   `/etc/resolv.conf` (dial-by-name from guests, ADR-0072 reused), FAIL-CLOSED
   (apply-or-`EXIT`, never exec). Beacon PL unchanged.
...
terminal. No test-only wiring. The Tier-3 AT boots a REAL microVM → gated
behind `kvm-tests` and run via `cargo xtask metal run --` (nested KVM; NOT
Lima). The AT additionally asserts three remediation obligations: **guest
dial-by-name** over routed hops (topology-reasoned, NOT spike-proven — the
spike used raw-IP connect — so the AT exercises it FIRST), **first-connect
safety** (the guest's first mesh `connect()` is captured with zero cleartext
SYN escaping — the born-captured invariant `install-success ≺ EXEC-release`),
and a **restart** AT (a restarted VM alloc re-installs the intercept via the
`:1880` gate and is driven terminal fail-closed on install failure).

**Reuse tally** (HARD GATE, full table in the feature-delta): 7 REUSE-AS-IS ·
7 EXTEND · 1 CREATE-NEW (the pure `VmTapPlan` value). Zero new crates, ports,
