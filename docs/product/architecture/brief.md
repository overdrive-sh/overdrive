...
| `KernelImage::validate`, `MemoryPlan::derive`, `DiskAttachment::to_disk_arg`, `VmConfinement::seccomp_arg`, `VmConfig::{landlock_rules, rlimit_fsize}` | **pure-function** | ∅ | Unit + proptest; all are Slice 01 mutation targets. **`rlimit_fsize` is pure only because `RootfsPlan` carries `master_bytes` captured at construction** — without that field it is a `stat(2)` wearing a pure signature |
