...
TPROXY/mark/accept semantics fire unchanged over a tap-fed veth. The
2026-08-29 Q9 amendment adds one passive anonymous counter between those
matches and actions. Full narrative:
...
7. **Kernel-observable alloc-rule accounting** (ADR-0088/0089): the existing
   ...
   unchanged alloc-host-veth/TCP matches and before the byte-identical
   TPROXY/mark/accept tail. `overdrive-netlink`'s internal `GETRULE` projection
   decodes that rule's packet+byte counts with bounded nested attribute walks.
   Shared, inbound, and output-divert rules remain unchanged.
...
`RuleCounterSnapshot { packets: u64, bytes: u64 }`; no API, Beacon,
persistence, observation, or describe schema changes.

While the guest is blocked before EXEC, two equal same-userdata+handle reads
across a capture-confirmed quiet interval define `before` and complete
intercept-live. After the flow, two equal quiet-interval reads of that same
identity define `after`. Checked packet+byte deltas must be positive and
bounded by the complete exact-host-veth capture's eligible-frame count and byte
total. The first eligible frame is the expected SYN and every eligible frame in
the window has its directional five-tuple; leg-F recovers that original
destination. Missing/duplicate/malformed counters, dump error/timeout,
instability, identity change, replacement, reset, regression, wrap/bound
violation, competing tuple, or capture loss fails conservatively. The metal
observer never installs, replaces, resets, or deletes. A same-tag adopt keeps
the accumulated value but takes a new baseline; restart boot sweep and
reinstall are never compared across handles; sibling rules/counters remain
untouched.
...
quiet-interval `GETRULE` reads established a stable counter baseline for the
exact full-userdata+handle outbound rule on that host-veth.
...
same directional tuple must cover every rule-eligible captured frame, and a
checked capture-bounded positive delta must occur on that same rule before the
original destination reaches leg-F. No cleartext copy may reach the external
peer path, and the inter-agent path must carry TLS records. This pins the
complete ordering
