...
Three additional design-shaping findings: (1) TLS 1.3 RX kTLS requires kernel ≥6.0. Overdrive ships its own stripped-down immutable appliance OS and **pins the kernel** (decided 2026-06-04, floor = **6.6 LTS**, latest LTS we ship), so kernel availability is a fixed target Overdrive controls — full in-kernel TLS 1.3 TX+RX *and* the kernel-side handshake API (`CONFIG_NET_HANDSHAKE`, ≥6.5) are both guaranteed present; there is no diverse-operator-kernel matrix to design fallbacks for. (2) TLS 1.3 KeyUpdate for mid-connection rekey landed only post-6.6, so on the 6.6 pinned kernel SVID rotation on long-lived connections still requires connection teardown and reconnect — though because Overdrive controls the kernel, pinning a newer kernel (or backporting the KeyUpdate patch) is a future option rather than an external constraint. (3) Cilium's "mutual auth" (the most visible prior art) does NOT use kTLS for encryption: it does an OOB mTLS handshake between agents, discards the keys, and relies on WireGuard/IPSec for encryption — their own open CFP #26480 acknowledges using session keys for kTLS as unimplemented future work.
...
**Limitation at the 6.6 pinned floor**: TLS 1.3 KeyUpdate landed post-6.6 and is NOT present on the pinned 6.6 kernel. SVID rotation mid-connection therefore requires **connection teardown and reconnect** (the node agent closes the connection when the SVID expires, forcing the application to reconnect and initiate a new handshake with the fresh SVID). This is the expected behaviour: SPIFFE SVIDs have 1h TTL; connections longer than 1h require a teardown/reconnect strategy. Because Overdrive owns the appliance kernel, advancing the pin to a KeyUpdate-bearing kernel (or backporting the patch) is a future option to enable in-place rekey — it is a roadmap choice, not an external blocker.
...
**Description**: kTLS TLS 1.3 KeyUpdate landed post-6.6 and is not present on the pinned 6.6 kernel. Long-lived connections (lasting > SVID TTL = 1h) cannot rotate keys in-place at the current pin. The option today is connection teardown and reconnect; advancing the appliance kernel pin to a KeyUpdate-bearing version (which Overdrive controls) is the path to in-place rekey later.
