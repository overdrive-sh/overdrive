# Research: HolmesGPT Integration Analysis for Helios

**Date**: 2026-04-19 | **Researcher**: nw-researcher (Nova) | **Confidence**: High | **Sources**: 18

## Executive Summary

**Recommendation**: Adopt the composition **Shape B + Shape D** — run HolmesGPT as a first-class Helios workload (persistent microVM + credential-proxy + content-inspector + BPF LSM) *and* ship a Helios-native MCP server exposing DuckLake, ObservationStore, IntentStore, incident memory, and `propose_action`. Optionally cherry-pick the HolmesGPT runbook format (markdown + `catalog.json`) for the Helios-native incident memory. **Reject** Shape A (embedding HolmesGPT in place of `rig-rs`) — it trades the single-binary/Rust-throughout/own-primitives claims for an ecosystem that Shapes B+D already unlock.

**Why this works**: HolmesGPT is not a novel orchestration framework — it is an Apache-2.0, Python, LiteLLM-backed agentic wrapper plus a curated catalog of 40+ toolsets and a markdown-runbook format. It is an MCP *client*. Helios already owns the matching Rust-native primitives (the `rig-rs` agent, DuckLake, ObservationStore, incident memory, graduated approval gate), and the §6/§8/§9 persistent-microVM + sidecar model is essentially *already describing this workload* — HolmesGPT is the flagship example. Exposing Helios as an MCP server gives every HolmesGPT deployment (embedded or external, in cluster or on an operator's laptop) a clean integration path that doesn't require Helios to grow a Python runtime.

**Key finding that flips the decision**: HolmesGPT is an MCP client, not an MCP server. Its own cloud integrations (AWS, Azure, Grafana, Splunk, GitHub, Kubernetes-Remediation) are MCP servers shipped as independent Helm sub-charts. A Helios MCP server is structurally indistinguishable from these — zero special-casing on the HolmesGPT side, zero Python on the Helios side.
...
**Search Strategy**: Canonical HolmesGPT sources — `github.com/robusta-dev/holmesgpt`, `holmesgpt.dev/latest/`, the project's operator docs, custom-toolset YAML spec, remote-MCP integration docs, runbooks reference, and GitHub Issues for production gotchas. Cross-referenced against Helios whitepaper v0.12 (especially §12 Observability and Self-Healing, §13 Dual Policy Engine, §18 Reconciler Model) to identify the capability delta.
...
**Quality Standards**: Target 2-3 sources per major HolmesGPT claim; every factual claim carries a URL; Helios-side claims are backed by whitepaper section numbers.
...
**Analysis for Helios**: Apache-2.0 is *one-way compatible* with Helios's AGPL-3.0 — Helios can embed or call Apache-2.0 code without license friction; any Helios-specific patches stay AGPL. No license blocker regardless of integration shape.
...
**Confidence**: High
**Analysis for Helios**: The useful frame is *HolmesGPT is an HTTP service plus an optional Kubernetes operator on top of that service.* The operator is a thin kopf controller translating CRDs into HTTP calls. The "service" is where the intelligence lives.
...
**Confidence**: High
**Analysis for Helios**: The 2 GiB floor and Python runtime cost matter if HolmesGPT runs co-located with control plane nodes; it also means HolmesGPT is a *substantial* Helios workload — bigger than the whole Helios control plane (which is ~30 MiB single / ~80 MiB HA per §4 of the whitepaper).
...
**Confidence**: High
**Analysis for Helios**: This is decisive. HolmesGPT does not itself provide a novel LLM-orchestration framework — it is an *opinionated tool/runbook catalog on top of LiteLLM*. Helios already has `rig-rs`, which is functionally equivalent at the provider-abstraction layer (Rust-native, supports OpenAI/Anthropic/Bedrock/local). The unique HolmesGPT asset is not the LLM wrapper — it is the **toolset ecosystem and runbook catalog**, plus the operator pattern.
...
**Confidence**: High
**Analysis for Helios**: Tools are **shell commands with Jinja-templated parameters that the LLM fills in**. This is a security-relevant design: the execution surface is arbitrary shell. HolmesGPT's mitigation is "read-only by default" RBAC + per-tool memory limits + output streaming. For Helios, if HolmesGPT runs as a workload, the existing BPF LSM (§19) `bprm_check_security` + `file_open` hooks are exactly the right secondary enforcement — a toolset trying to execute a binary outside an allowlist would be blocked kernel-side regardless of what the LLM decides to do.
...
**Confidence**: High
**Analysis for Helios**: This catalog is the ecosystem moat. Rebuilding it in Rust would be a multi-quarter effort for catch-up value. The YAML format is simple enough that a Rust loader is feasible, but the *maintenance burden* of tracking upstream tool/API changes is the real cost.
...
**Confidence**: High
**Analysis for Helios**: This is strictly document-and-retrieve, not a DAG or workflow DSL. It does not compete with Helios's workflow reconciler (§18) — the workflow reconciler executes durable multi-step operations; HolmesGPT runbooks only *guide the LLM's reasoning*.
...
**Confidence**: High
**Analysis for Helios**: The operator pattern is **Kubernetes-shaped**: CRDs for declarative "health check" objects reconciled by a controller. Helios explicitly rejects this pattern (§1 Motivation — "CRDs and operators are the extension model — Go binaries that run with cluster-admin privileges and frequently destabilize production clusters"). The Helios equivalent is a **reconciler** (§18). HolmesGPT's operator value is low on Helios — it is translating `scheduled` → `run this investigation` in Kubernetes idiom; Helios's reconciler model gives the same result natively.
...
**Confidence**: High
**Analysis for Helios**: HolmesGPT is an **MCP client, not an MCP server**. This is the pivotal finding for integration shape: Helios does not need to "run HolmesGPT" to benefit from HolmesGPT's investigation ability — Helios only needs to **expose itself as an MCP server**, and then any HolmesGPT deployment (inside or outside the cluster) can query it.
...
**Confidence**: High
**Analysis for Helios**: The default posture is exactly what Helios §12 already aims for at the LLM tier — the LLM proposes, the approval gate ratifies. Helios's equivalent lives in `propose_action(action)` feeding the graduated approval gate.


## 2. Capability Overlap With Helios §12
...
| Capability | Helios (§ reference) | HolmesGPT | Delta |
|---|---|---|---|
| LLM provider abstraction | `rig-rs` (Rust, §12) | LiteLLM (Python) | **Covered** — both abstract the same providers; `rig-rs` is the Helios-native answer |
| Agentic tool-calling loop | `rig-rs` agent with `query_flows`, `get_job_status`, `get_policy_decisions`, `get_node_metrics`, `get_incident_history`, `propose_action` (§12) | Custom `ToolCallingLLM` wrapper over LiteLLM, `max_steps` ceiling, context compaction | **Covered** — both are agentic loops; Helios's is narrower but Rust-native |
| Structured, identity-tagged telemetry ingress | eBPF `FlowEvent` + `ResourceEvent` with full SPIFFE IDs (§12) | Relies on external observability (Prometheus/Loki/Datadog) | **Helios advantage** — kernel-native, zero-instrumentation, identity-bearing |
| SQL-queryable telemetry store | DuckLake (libSQL catalog + Parquet in Garage, §17) with time travel | Tools query external stores per-investigation | **Helios advantage** — single SQL endpoint vs N integrations |
| Tiered self-healing | Tier 1 reflexive eBPF, Tier 2 reconciler, Tier 3 LLM reasoning (§12) | Tier 3 equivalent only (HolmesGPT is the reasoning tier, nothing else) | **Helios advantage** — HolmesGPT has no Tier 1/Tier 2 story |
| Incident memory with similarity search | libSQL + embedding-based similarity search of past incidents (§12) | Not shipped — runbooks are the nearest analogue | **Helios advantage** — accumulated diagnostic memory |
...
| MCP client/server | Neither in whitepaper | MCP client (+ several own integrations exposed as MCP servers) | **HolmesGPT advantage** — but Helios could easily be an MCP server |
...
| Chaos injection to validate healing | Chaos reconciler (§18), chaos sidecar (§9) | None | **Helios advantage** |
...
**What is genuinely new from HolmesGPT's perspective that Helios does not already have**:
...
**What Helios already covers cleanly**:
...
Each shape is evaluated against Helios design principles **1** (own your primitives), **7** (Rust throughout — no FFI to Go or C++ in the critical path), **5** (observability is native, not retrofitted), and against the existing §12 architecture.

### Shape A — Replace the `rig-rs` agent with embedded HolmesGPT

**What it means**: Remove the `rig-rs`-based §12 LLM agent; embed a HolmesGPT Python runtime in the control plane; translate the existing Helios tools into HolmesGPT toolsets.
...
| Operational cost | Adds Python + 2 GiB RAM floor to the control plane. Helios control plane is ~30 MiB / ~80 MiB today. |
...
**Verdict**: **Reject.** This shape sacrifices the single largest architectural claim of Helios to buy an ecosystem that can be accessed without embedding.

### Shape B — Run HolmesGPT as a first-class Helios *workload*

**What it means**: Package the HolmesGPT HTTP server as a persistent microVM (§6). Declare `persistent = true`, attach the `builtin:credential-proxy` sidecar (§8) for LLM provider keys, attach `builtin:content-inspector` (§9) for the content returned by investigations, expose it through the gateway (§11) as a private service VIP. HolmesGPT's MCP client reaches back into Helios through an MCP server that the control plane exposes (Shape D composes with this).

| Dimension | Assessment |
|---|---|
| Principle 1 (own primitives) | **Honored**. HolmesGPT is just another workload; Helios owns the substrate. |
| Principle 7 (Rust throughout) | **Honored in Helios code**; HolmesGPT is Python but runs as a guest, not as Helios-internal code. |
| Principle 5 (observability native) | **Honored** — HolmesGPT queries Helios via MCP against DuckLake/ObservationStore; the foundation stays eBPF-native. |
...
**Verdict**: **Strong fit.** This is Helios eating its own dog food. The whitepaper already contains this pattern; HolmesGPT is the flagship third-party example.
...
| Toolset execution model | **Caveat**: HolmesGPT toolsets shell out to commands (`kubectl`, `curl`, etc.). Either Helios ships those binaries in the Image Factory (§24, contradicting the minimal-OS claim) or the toolsets run in a sidecar/VM anyway — in which case Shape B wins. |
...
**Verdict**: **Partial adopt.** The *runbook format* is cheap to ingest and valuable on its own. The *toolset execution model* (shell-out-with-Jinja-templated-params) fights the Helios Image Factory discipline — don't port it wholesale; cherry-pick runbooks.

### Shape D — Expose Helios as a HolmesGPT toolset (MCP server)

**What it means**: Helios ships a built-in MCP server (streamable-HTTP transport) exposing Helios-native tools:
- `helios.query_flows(sql)` → DuckLake SQL
- `helios.query_observation(sql)` → ObservationStore SQL
- `helios.get_alloc_status(job_id)` / `helios.list_nodes()` / `helios.list_policies()`
- `helios.get_incident(id)` / `helios.search_incidents(embedding_query)`
- `helios.propose_action(action)` — returns the approval-gate token, does **not** execute until ratified

Any HolmesGPT deployment — embedded (Shape B) or external (operator's cluster, laptop, hosted) — consumes these as a remote MCP server. Helios is agnostic to where HolmesGPT runs.

| Dimension | Assessment |
|---|---|
| Principle 1 | **Honored** — MCP is an open protocol; Helios ships its *own* server, not an embedded client. |
...
| Interop value | **High**. Any MCP client (HolmesGPT, Claude Desktop, Cursor, future SRE tools) can query Helios. The protocol is becoming industry default in 2026. |
| Security | The MCP server sits behind the gateway (§11), uses the same mTLS + SPIFFE identity as every other Helios workload. Clients authenticate as regular workloads. |

**Verdict**: **Strong fit, independent of Shape B.** This is the *protocol-level* answer: Helios becomes a first-class citizen of the MCP ecosystem without taking on Python or HolmesGPT dependencies.

### Recommended composition

**Shape B + Shape D + (optional) Shape C-for-runbooks-only.**

- **B** lets operators who want HolmesGPT *run it the Helios way* — persistent microVM, credential-proxy, content-inspector, behind the gateway, under BPF LSM. The whitepaper's Persistent MicroVMs section already implies this; HolmesGPT becomes the first-class example.
- **D** makes Helios a proper MCP server so that HolmesGPT (wherever deployed) and the broader MCP ecosystem reach Helios data cleanly, without Helios growing a Python runtime.
- **C (runbooks only)** is optional — adopt the markdown + `catalog.json` format for the Helios-native incident-memory subsystem (§12) so incident learnings can be published as runbooks that both the `rig-rs` agent and HolmesGPT can consume.
...
### 4.1 Helios subsystems that supply data to HolmesGPT

All of the following surface through the Helios MCP server (Shape D). HolmesGPT in a persistent microVM (Shape B) consumes them over MCP like any other MCP client.

| Helios subsystem | MCP tool exposed | Backed by |
|---|---|---|
| Flow telemetry | `helios.query_flows(sql)` | DuckLake (§12, §17) — libSQL catalog + Parquet in Garage, time-travel supported |
| Resource telemetry | `helios.query_resources(sql)` | DuckLake (§17) |
| Live allocation status | `helios.query_allocations(sql)` | ObservationStore → Corrosion/cr-sqlite `alloc_status` table (§4) |
| Service backend map | `helios.query_services(sql)` | ObservationStore `service_backends` (§4) |
| Node health | `helios.query_nodes(sql)` | ObservationStore `node_health` (§4) |
| Compiled policy verdicts | `helios.query_policy_verdicts(sql)` | ObservationStore `policy_verdicts` (§4) |
| Job specs, policies, certs | `helios.get_job(id)`, `helios.get_policy(id)` | IntentStore (§4) — read-only projection |
| Past incidents | `helios.search_incidents(text_or_embedding)` | Incident memory libSQL with embedding similarity (§12) |
| Kernel-level events | `helios.tail_events(alloc_id, duration)` | eBPF ringbuf events via the telemetry pipeline (§12) |
...
### 4.2 Helios subsystems that consume HolmesGPT output

| Helios subsystem | How HolmesGPT output flows in | Enforcement |
|---|---|---|
| Approval gate (§12 Tier 3) | `helios.propose_action(action)` returns an approval token; action is queued, not executed | Graduated gate based on action risk — low-risk (cert rotation, right-size) auto-ratifies, high-risk (workload stop, policy change) requires operator confirmation |
| Workflow reconciler (§18) | Multi-step HolmesGPT remediations map to a Helios `Workflow` — each HolmesGPT step becomes a durable workflow `await` point | Crash-safe resume; replayable journal |
| Incident memory (§12) | Every HolmesGPT investigation (prompt, tool calls, verdict, outcome) is persisted to the libSQL incident store | Embedding similarity search lets future investigations (Helios-native or HolmesGPT) retrieve the prior outcome |
...
config.allowed_domains = [
  ...
  "prometheus.helios.local",
  "grafana.helios.local",
]
...
### 4.4 How HolmesGPT runbook execution maps to Helios primitives

HolmesGPT's runbook flow:
1. Incident arrives (HolmesGPT pulls from AlertManager/PagerDuty, or Helios pushes via the MCP server).
...
| HolmesGPT step | Helios equivalent |
|---|---|
...
| Step 3 — fetch runbook | Runbook markdown lives in Garage (content-addressed); `helios.get_runbook(id)` MCP tool |
| Step 4 — diagnostic commands | MCP tools — no shell-out needed, all diagnostics are SQL against DuckLake/ObservationStore |
| Step 5 — remediation | `helios.propose_action(action)` → graduated approval gate (§12 Tier 3) → workflow reconciler (§18) executes the action durably |

### 4.5 How HolmesGPT's toolset pattern composes with Helios primitives

HolmesGPT toolsets *do not map cleanly* onto Helios reconcilers (reconcilers are level-triggered convergence loops, not request handlers) or onto Helios sidecars (sidecars sit in the data path, not the control path). The closest Helios primitive is the **MCP tool** — a named, JSON-schema'd, auth-gated RPC.
...
- Helios-native diagnostic tools (DuckLake SQL, ObservationStore SQL, incident search) are exposed as MCP tools (Shape D). HolmesGPT consumes them alongside its existing toolsets.
- The *runbook catalog* is shared — a single markdown+catalog.json corpus in Garage, readable by both the `rig-rs` agent (Helios-native, §12) and HolmesGPT (external or microVM).

## 5. MCP Angle — Validated

**Claim**: Helios can be consumed by HolmesGPT *without any Helios-side HolmesGPT-specific code* if Helios exposes an MCP server.
...
- HolmesGPT is an MCP client. Remote MCP server integration is a first-class, documented configuration surface. The YAML form is:
  ...
  mcp_servers:
    helios:
      description: "Helios cluster telemetry and state"
      config:
        url: "https://helios-mcp.internal/mcp/messages"
        mode: streamable-http
        headers:
          Authorization: "Bearer {{ env.HELIOS_MCP_TOKEN }}"
  ```
...
**Implication**: Shape D is not a forced fit — it is the path HolmesGPT's own first-party integrations take. A Helios MCP server would be structurally indistinguishable from `mcp-aws`, `mcp-azure`, `mcp-grafana`.

**Protocol version caveat**: the HolmesGPT docs do not pin an MCP protocol version. A Helios MCP server should follow the MCP spec at the revision HolmesGPT's current release targets and test against both recent HolmesGPT minor versions.
...
**Implication for Helios**: Cost accounting is the operator's responsibility. Helios should treat HolmesGPT as an LLM-metered workload and meter outbound LLM API spend via the credential-proxy (§8) — the proxy already sees every outbound request; adding per-workload token-cost accounting from the rate-limiter sidecar (§9 `builtin:rate-limiter`) is the correct place.
...
**Confidence**: Medium
**Implication for Helios**: Helios already has the right structural answer in §12 — every LLM-proposed action flows through the graduated approval gate; low-risk actions may auto-ratify, high-risk actions require operator review. If HolmesGPT runs as a Helios workload (Shape B), its `propose_action`-equivalent calls land on the same approval gate. Hallucination becomes an operator-review problem for actions, not a correctness problem for reads (which are read-only by design).
...
**Confidence**: High
**Implication for Helios**: Memory protection is a solved problem inside HolmesGPT. For Shape B, cgroup-level memory caps (§14 live right-sizing; also the VM's `resources.memory_bytes`) provide the outer ring that even a bug in HolmesGPT's own budget logic cannot exceed.
...
**Confidence**: High
**Implication for Helios**: Shape B runs HolmesGPT in a microVM with its own rootfs (via `helios-fs`, §17) — the `meta-helios` Yocto layer stays Python-free. Shape A would drag Python into `meta-helios`; this is the concrete expression of the Principle 7 objection.
...
**Implication for Helios**: This reinforces the §12 tiered-healing model. HolmesGPT fits Tier 3 (seconds to minutes). Tier 1 (milliseconds, eBPF) and Tier 2 (seconds, reconciler) remain Helios-native, non-LLM paths. Do not put HolmesGPT on Tier 1 or Tier 2.

### 6.7 DST gap — the LLM tier is not simulation-testable today

**Evidence**: Helios §21 enumerates `SimClock`, `SimTransport`, `SimEntropy`, `SimDataplane`, `SimDriver`, `SimObservationStore` — the LLM agent is not in the trait list. HolmesGPT has an evals framework but it is not deterministic-simulation shape.
**Source**: Helios whitepaper §21; project eval framework references in [CNCF blog](https://www.cncf.io/blog/2026/01/07/holmesgpt-agentic-troubleshooting-built-for-the-cloud-native-era/)
**Confidence**: High
**Implication for Helios**: A `SimLLM` trait (returning fixed, seed-driven completions) should be added for DST coverage of the control-plane path that invokes the LLM agent. This is a genuine gap in both projects; Helios is better positioned to close it because DST is already a first-class discipline in §21.
...
> Helios exposes its observability and state layers through an in-process **MCP (Model Context Protocol) server**, part of the gateway subsystem (§11). The server publishes a fixed toolset mapping directly onto the stores:
>
> | MCP tool | Backend |
> |---|---|
> | `helios.query_flows(sql)` · `helios.query_resources(sql)` | DuckLake (§17), time-travel supported |
> | `helios.query_allocations(sql)` · `helios.query_services(sql)` · `helios.query_nodes(sql)` · `helios.query_policy_verdicts(sql)` | ObservationStore (§4) |
> | `helios.get_job(id)` · `helios.get_policy(id)` · `helios.get_route(id)` | IntentStore (§4), read-only projection |
> | `helios.search_incidents(query)` | Incident memory libSQL with embedding similarity (§12) |
> | `helios.tail_events(alloc_id, duration)` | eBPF ringbuf via the telemetry pipeline (§12) |
> | `helios.propose_action(action)` | Graduated approval gate (§12 Tier 3); returns a token, never executes inline |
>
> Clients authenticate as regular Helios workloads. In-cluster consumers carry SVIDs and reach the MCP server through east-west mTLS; external clients authenticate via gateway-issued mTLS or operator-minted API tokens stored in the IntentStore.
>
> **HolmesGPT — the reference MCP consumer.** Helios's reasoning tier is built on `rig-rs`. It does not need to be replaced. Operators who additionally want HolmesGPT (CNCF Sandbox, Apache-2.0) — its 40+ toolsets for Prometheus, Grafana, cloud providers, databases, runbook matching, alerting-platform integration — run HolmesGPT as a first-class Helios workload:
>
> ```toml
> [job]
> name   = "holmesgpt"
...
- **§23 Roadmap, Phase 5**: add a bullet — "MCP server (streamable-HTTP transport) exposing DuckLake, ObservationStore, IntentStore, incident memory, and `propose_action` as Helios's Tier 3 integration surface."
- **§23 Roadmap, Phase 6**: add — "HolmesGPT as a reference persistent-microVM workload shipped via the Image Factory (§24), validating that external AI agents can run on Helios with credential-proxy + content-inspector + BPF LSM as the structural security ring."
...
| Helios whitepaper v0.12 (§1–24) | local SSOT | High | Primary source of record | 2026-04-19 | N/A (authoritative) |
...
**Recommendation**: Before productionising Shape B, run a Helios-local eval suite against HolmesGPT v0.24.3 under representative Helios incident scenarios (policy denial, OOM, cert rotation failure) and publish the numbers. Meter via the credential-proxy + rate-limiter sidecars.

### Gap 2: MCP protocol version pinning
**Issue**: HolmesGPT's MCP client docs do not pin a protocol version. A Helios MCP server needs to target a specific MCP spec revision.
...
**Recommendation**: Moot for the recommendation (Shape B doesn't use the operator — Helios's own reconciler/workflow model replaces it). If Shape B ever wants the `ScheduledHealthCheck` pattern, implement it as a Helios reconciler reading a new intent kind, not by porting the kopf operator.
...
[18] Helios project. "Helios Whitepaper v0.12". Local SSOT. Accessed 2026-04-19. `/Users/marcus/conductor/workspaces/helios/taipei-v1/docs/whitepaper.md`.
