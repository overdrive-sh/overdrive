...
### Finding 7 — K8s VPA + in-place is the closest industry analogue to Overdrive §14
...
**What's missing vs Overdrive §14**: K8s VPA polls metrics-server at fixed intervals (minutes). It cannot *prevent* OOM — it reacts to metrics. Overdrive §14 describes eBPF-observed pressure with **pre-OOM live resize**, which is structurally different (sub-second reaction, in-kernel signal). No K8s VPA equivalent for eBPF-driven OOM-predictive resize was found.
...
**Consequence**: Process driver right-sizing on Overdrive does not depend on Cloud Hypervisor. Any claim that §14's process-workload right-sizing requires Cloud Hypervisor is confused — it requires cgroups v2.
...
| eBPF-observed pressure → pre-OOM cgroup write | ✅ Plausible, real novelty | K8s VPA does not use eBPF pressure signals; they poll metrics-server. The eBPF-driven path is a genuine Overdrive differentiator for process workloads. |
...
### Does Overdrive find a real gap? Yes — but a narrow one:
...
- Stateful workloads where snapshot/restore cost is high (Overdrive's persistent microVM target)
- Workloads where even a few hundred ms of downtime is a hard violation (rare)

### Verdict on §14's load-bearing status
- The **process/container** half is genuinely cgroup-driven and does not depend on Cloud Hypervisor. If Overdrive dropped VM workloads entirely, §14 would still work for processes. Defensible but converging with industry (K8s 1.35).
...
2. **Soften the uniqueness rhetoric.** KubeVirt (via live migration) and VMware (via hot-add) already do VM-level in-place resize. K8s 1.35 GA (Dec 2025) reached in-place for containers. The Overdrive novelty is:
...
3. **Acknowledge restart-based is usually fine.** Most production VM resize *is* done by restart, and this works. Overdrive's differentiator is the long-lived stateful VM case (persistent microVM, §6) where restart is expensive. Explicitly tie §14 to that case.
...
1. **No public evidence of production eBPF-driven right-sizing beyond monitoring**. Netflix/Stripe/Cloudflare have published on eBPF but not on "eBPF pressure → cgroup adjust before OOM" as a production loop. Overdrive's pattern appears genuinely novel — which sometimes means "tried and abandoned."
