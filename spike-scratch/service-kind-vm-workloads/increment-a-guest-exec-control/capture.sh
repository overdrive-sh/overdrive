...
readonly SANITIZED_COMMAND="RSYNC_BIN=$INCREMENT/rsync-without-local-env.sh OVERDRIVE_METAL_KERNEL=/srv/vm/overdrive-testing/kernel OVERDRIVE_METAL_ROOTFS=/srv/vm/overdrive-testing/rootfs.ext4 OVERDRIVE_METAL_SCENARIO=service-vm-h1-h5-rerun cargo xtask metal run -- bash -lc 'cargo build --manifest-path $INCREMENT/Cargo.toml --release --target x86_64-unknown-linux-musl && bash $INCREMENT/run.sh'"
readonly RUN_ID="${1:?usage: capture.sh RUN_ID (for example 0002)}"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/service-vm-h1-h5-capture.XXXXXX")"
...
OVERDRIVE_METAL_SCENARIO=service-vm-h1-h5-rerun \
cargo xtask metal run -- bash -lc \
  "cargo build --manifest-path $INCREMENT/Cargo.toml --release --target x86_64-unknown-linux-musl && bash $INCREMENT/run.sh" \
...
{
  printf 'attempt=tracked-harness-rerun-%s\n' "$RUN_ID"
