...
readonly SANITIZED_COMMAND="RSYNC_BIN=$INCREMENT/rsync-without-local-env.sh OVERDRIVE_METAL_KERNEL=/srv/vm/overdrive-testing/kernel OVERDRIVE_METAL_ROOTFS=/srv/vm/overdrive-testing/rootfs.ext4 OVERDRIVE_METAL_SCENARIO=service-vm-h6-rerun cargo xtask metal run -- bash -lc 'cargo build --manifest-path $INCREMENT/Cargo.toml --release --target x86_64-unknown-linux-musl && bash $INCREMENT/run.sh'"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/service-vm-h6-capture.XXXXXX")"
...
OVERDRIVE_METAL_SCENARIO=service-vm-h6-rerun \
