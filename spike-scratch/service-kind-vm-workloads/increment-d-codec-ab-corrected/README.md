# Increment D: corrected VM-Exec codec A/B
...
version inside the bounded payload, and a 16 KiB declared-length cap checked
before codec decoding. The
...
```sh
bash spike-scratch/service-kind-vm-workloads/increment-d-codec-ab-corrected/capture.sh 0001
```
...
Increment C is preserved as failed evidence. Its first runtime run exposed a
fixture quoting defect and, more importantly, that the `rkyv` feature build had
dead-code-eliminated the baseline Beacon JSON path. This corrected increment
executes that baseline path in all three variants before measuring either
candidate codec, so every comparison includes the JSON code already active in
the real init binary.
...
The harness receives complete frames as byte slices; it proves oversized
declared lengths are rejected before codec decoding, not that a future stream
reader performs the check before allocating its payload buffer.
