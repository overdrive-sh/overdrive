...
spike_links+=(br295p)
ip link set br295p address 02:00:00:95:00:01
...
nft add rule bridge gh295p prerouting iifname tap295a ether type ip ip protocol tcp tcp dport 9000 counter meta nftrace set 1 meta mark set 0x295a ether daddr set 02:00:00:95:00:01 meta pkttype set host
...
nft add rule ip gh295p prerouting meta mark 0x295a ip protocol tcp tcp dport 9000 counter tproxy to 127.0.0.1:15294 meta mark set 0x295 accept
...
timeout 35 cloud-hypervisor --cpus boot=1 --memory size=256M --kernel "$OVERDRIVE_METAL_KERNEL" --cmdline 'root=/dev/vda rw console=ttyS0 init=/init panic=0 loglevel=4 spike_role=client' --disk "path=$spike_run/client.ext4,image_type=raw" --net 'tap=tap295a,mac=02:00:00:95:00:02' --serial "file=$spike_run/client.console" --console off > "$spike_run/client.stderr" 2>&1 &
spike_client=$!
...
print('VERDICT=WORKS: two real microVMs, guest plaintext DNS dial, per-port bridge-local+TPROXY, mutual TLS1.3 with kTLS TX/RX, zero cleartext on leg-B/leg-C, no direct shared-L2 bypass')
PCAP
