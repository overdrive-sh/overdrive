...
cleanup() {
  ...
  ip -br addr
  ip rule
  for f in host.log client.console server.console client.stderr server.stderr capture-lo.stderr capture-tap295a.stderr capture-tap295b.stderr; do
...
nft add rule bridge gh295p prerouting iifname tap295a ether type ip ip protocol tcp tcp dport 9000 counter meta broute set 1
nft add rule bridge gh295p prerouting iifname tap295a ether type ip ip protocol udp udp dport 53 counter meta broute set 1
...
timeout 40 cloud-hypervisor --cpus boot=1 --memory size=256M --kernel "$OVERDRIVE_METAL_KERNEL" --cmdline 'root=/dev/vda rw console=ttyS0 init=/init panic=1 loglevel=4 spike_role=server' --disk "path=$spike_run/server.ext4" --net 'tap=tap295b,mac=02:00:00:95:00:03' --serial "file=$spike_run/server.console" --console off > "$spike_run/server.stderr" 2>&1 &
spike_server=$!
...
timeout 35 cloud-hypervisor --cpus boot=1 --memory size=256M --kernel "$OVERDRIVE_METAL_KERNEL" --cmdline 'root=/dev/vda rw console=ttyS0 init=/init panic=1 loglevel=4 spike_role=client' --disk "path=$spike_run/client.ext4" --net 'tap=tap295a,mac=02:00:00:95:00:02' --serial "file=$spike_run/client.console" --console off > "$spike_run/client.stderr" 2>&1 &
spike_client=$!
