//! PROBE increment-e — guest PID 1 for P6 (virtiofsd + `--memory shared=on`),
//! **rebuilt for env B: bare-metal x86_64, no nested virtualisation.**
//!
//! increment-d ran this logic on nested aarch64, where `--memory shared=on`
//! never boots (0/12), so every line below the mount attempts was unreachable
//! and P6's round-trip half stayed unproven. Same probe; what changed is that
//! on env B it can actually run to completion.
//!
