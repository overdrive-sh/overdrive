...
This checked-in product example is one VM-only bundle supporting the healthy
Service journey and its bounded HTTP/TCP probe and lifecycle controls. The
healthy Service declares wildcard TCP startup and omitted-host HTTP readiness,
both of which reach the guest address provisioned by Overdrive.
...
`guest_server.rs` is a dependency-free VM workload. TCP/18081 answers the raw
backend path; HTTP/18080 supports fixed 204/302/404/503 startup results, timed
readiness 204→503→204, timed liveness failure, and the byte-exact
`SVM-E08-GUEST-OK` body at `/`.
...
with the accepted kernel/rootfs fields and guest client command. It also checks
the VM Service fixtures used by the probe and lifecycle controls. On qualified
native metal, `prepare.sh prepare`:
