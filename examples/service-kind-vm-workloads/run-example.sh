...
run_liveness_restart() {
  ...
  [[ "$replacement_alloc" != "$original_alloc" ]] \
    || die "E12 restart did not allocate a distinct successor identity"
  ...
  echo '--- E12 ledger end ---'
  echo 'E12 PASS: liveness stop, fresh-successor replacement, no readiness restart, no dead revival'
...
copy_case_captures() {
  ...
  find "$OUTPUT_ROOT" -maxdepth 1 -type f -name '*.log' \
    -exec cp -- {} "$SVM_E08_CASE_CAPTURE_DIR" \;
}
...
case_cleanup() {
  ...
  if [[ -n "$SNAPSHOT_DIR" ]]; then
    if [[ -n "${SVM_E08_CASE_CAPTURE_DIR:-}" ]]; then
      ...
      report_cleanup_deltas >"$SVM_E08_CASE_CAPTURE_DIR/cleanup-resource-delta.log" 2>&1 || failed=1
    else
      report_cleanup_deltas || failed=1
    fi
    rm -rf -- "$SNAPSHOT_DIR"
...
run_case() {
  ...
  if [[ -n "${SVM_E08_CASE_CAPTURE_DIR:-}" ]]; then
    ...
    printf '%s\n' 'vm-service-capture' >"$SVM_E08_CASE_CAPTURE_DIR/capture-token.log"
  fi
